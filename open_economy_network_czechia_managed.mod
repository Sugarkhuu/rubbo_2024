...
// Rubbo (2024) extended to a Small Open Economy -- CZECHIA DATA CALIBRATION
...
// reverse flows) taken from Czechia's actual 2022 Eurostat symmetric
// input-output table -- see data_calibration/build_czechia_calibration.py
// and data_calibration/czechia_calibration_results.json for the full
// source, sector concordance, and derivation. Because Omega^H is no
// longer nilpotent, (I-Omega^H)^-1 is computed here via the general 3x3
// matrix-inverse (adjugate/cofactor) formula, written out as scalar
// Dynare parameter algebra so the whole calibration stays self-contained
// and auditable (no hidden external matrix-inversion step).
//
// CALIBRATED FROM DATA (Eurostat naio_10_cp1700, Symmetric input-output
// table at basic prices, product by product, geo=CZ, time=2022):
// OH11-OH33 (domestic IO matrix), OF1-OF3 (import cost shares), BH1-BH3
// (household consumption shares), ALPHA1-ALPHA3 (value-added shares,
// standing in for the model's labor share since there is no separate
// capital factor).
...
parameters
  ...
  ALPHA1 ALPHA2 ALPHA3          // value-added shares (Czechia IO data)
  OH11 OH12 OH13                // domestic IO shares, Resource buyer  <- R,M,S seller (Czechia IO data)
  OH21 OH22 OH23                // domestic IO shares, Manuf.  buyer  <- R,M,S seller
  OH31 OH32 OH33                // domestic IO shares, Services buyer <- R,M,S seller
  OF1 OF2 OF3                    // total imported-input cost shares (Czechia IO data)
  BH1 BH2 BH3                    // domestic consumption shares (Czechia IO data)
...
// Value-added shares: DATA (Eurostat naio_10_cp1700, TOTAL flow, row "B1G"
// Gross value added, aggregated Resource/Manuf/Services).
ALPHA1  = 0.4008;
ALPHA2  = 0.2559;
ALPHA3  = 0.5486;

// Domestic IO matrix OH_ij = buyer i's cost share spent on seller j: DATA
// (Eurostat naio_10_cp1700, stk_flow=DOM, aggregated & renormalized so
// ALPHA_i + sum_j OH_ij + OF_i = 1 exactly -- see build script).
OH11 = 0.1244;  OH12 = 0.1667;  OH13 = 0.1521;   // Resource buys from R, M, S
OH21 = 0.0207;  OH22 = 0.2624;  OH23 = 0.1432;   // Manuf.   buys from R, M, S
OH31 = 0.0035;  OH32 = 0.0672;  OH33 = 0.2798;   // Services buys from R, M, S

// Import cost shares: DATA (Eurostat naio_10_cp1700, stk_flow=IMP).
OF1     = 0.1560;    // Resource
OF2     = 0.3178;    // Manufacturing (most import-intensive sector -- Czechia's manufacturing
                     // runs on deep German/EU automotive & machinery supply chains)
OF3     = 0.1009;    // Services

// Household consumption shares: DATA (Eurostat naio_10_cp1700, TOTAL flow,
// column "P3_S14" Household final consumption expenditure).
BH1     = 0.0312;
BH2     = 0.3144;
BH3     = 0.6544;
BF_TOT  = 0.10;
...
// against data_calibration/build_czechia_calibration.py's numpy computation
// (LAMBDA_D = [0.0523, 0.5314, 1.0254], MIMP = [0.3009, 0.4754, 0.1859]).
// -------------------------------------------------------------------------
M11 = 1 - OH11;
M12 = -OH12;
M13 = -OH13;
