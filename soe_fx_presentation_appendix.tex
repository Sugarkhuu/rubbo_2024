...
\midrule
$\psi$ & Elasticity of the debt-elastic risk premium to NFA deviations from $\bar b^*$ & 0.020 & \textbf{Key mechanism}: how hard external borrowing costs bite when NFA moves; drives Peg's dominant loss channel \\
...
\item \textbf{Float/Managed:} all positive, concentrated in Services (highest Domar weight, $\lambda_{D,\text{Services}}{=}1.11$) --- the network raises price-dispersion cost the most where network centrality is highest
\item \textbf{Peg:} Resource $\approx0$, Services mildly positive, \textbf{Manufacturing negative} --- net premium across sectors is close to zero
\item \textbf{Why Peg looks different:} the network doesn't eliminate cost-push, it \textbf{redirects} it (see Mechanism slide) --- with no interest-rate room under a peg, the extra propagation shows up in the \textbf{output gap} instead of price dispersion (Peg's Services output-gap variance: $8.97\to35.16$, network OFF vs.\ ON, Network Exposure slide) --- this panel being flat for Peg is exactly what tells you to look at the output-gap channel instead
\end{itemize}}
