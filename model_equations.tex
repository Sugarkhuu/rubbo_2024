...
\begin{equation}
P^C_tC_t+\frac{B_{t+1}}{1+i_t}+S_t\frac{B^*_{t+1}}{1+i^{*,\text{eff}}_t}
...
\begin{equation}
i^{*,\text{eff}}_t\;=\;i^*-\psi\!\left(\frac{S_tB^*_t}{P^C_t}-\bar b^*\right),
\quad\psi>0,
\label{eq:premium}
\end{equation}
...
The model is implemented in the \texttt{.mod} file below.
The free float is the default; switching to the peg or managed float
requires uncommenting the relevant \texttt{@\#define REGIME} line.
Run with Dynare 5.x: \texttt{dynare open\_economy\_network.mod}.

\begin{lstlisting}[caption={open\_economy\_network.mod --- full Dynare code},label={lst:dynare}]
// =========================================================
