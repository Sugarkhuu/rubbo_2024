\documentclass[aspectratio=43, 10pt]{beamer}
\title{\textbf{Exchange rate and Production network}}
\subtitle{Macro Internal Seminar} % Gomes, Haliossos, and Ramadorai (2021)
\author{Sugarkhuu Radnaa}
\institute[]{Bonn Graduate School of Economics}
\date{July, 2026}
\usetheme{default}
\usepackage{ amssymb }
\usepackage{xcolor}
\setbeamercolor{normal text}{fg=black}
\setbeamercolor{title}{fg=black}
\setbeamercolor{subtitle}{fg=black}
\setbeamercolor{itemize item}{fg=customgreen}
\setbeamertemplate{frametitle}[default][left]
\setbeamertemplate{items}[circle]
\definecolor{customgreen}{RGB}{255, 255, 255} %{10, 43, 19}
\setbeamercolor{frametitle}{bg=customgreen, fg=black}
\setbeamerfont{frametitle}{size=\Large, series=\bfseries}
\setlength{\headheight}{1.5cm} % Adjust as needed
\usepackage{natbib}
\usepackage{mathtools}
\usepackage{bm}

\setbeamercolor{author in head/foot}{fg=black,bg=white}
% Remove navigation symbols
\setbeamertemplate{navigation symbols}{}

% Footer: title left, page number right
\setbeamertemplate{footline}{
  \leavevmode%
  \hbox{%
    \begin{beamercolorbox}[wd=.8\paperwidth,ht=2.5ex,dp=1.5ex,leftskip=1em]{author in head/foot}%
      \scriptsize\insertshorttitle
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.2\paperwidth,ht=2.5ex,dp=1.5ex,rightskip=1em plus1fil]{author in head/foot}%
      \scriptsize\hfill\insertframenumber{} / \inserttotalframenumber
    \end{beamercolorbox}%
  }%
}

% -- required by the deck's IRF plots, tables, and backup-slide numbering
% (kept even though the rest of the template above is simplified/plain)
\usepackage{booktabs}
\usepackage{tikz}
\usepackage{pgffor}
\usepackage{pgfplots}
\usepackage{appendixnumberbeamer}
\pgfplotsset{compat=1.18}
\usetikzlibrary{arrows.meta, positioning}
\hypersetup{colorlinks=false}

% -- sector/shock colors used throughout the IRF and mechanism slides
\definecolor{sectorres}{RGB}{124, 58, 237}
\definecolor{sectorman}{RGB}{217, 119, 6}
\definecolor{sectorserv}{RGB}{8, 145, 178}
\definecolor{riskred}{RGB}{220, 38, 38}

\newcommand{\bpi}{\bm{\pi}}
\newcommand{\ty}{\tilde{y}}
\newcommand{\tA}{\tilde{A}}
\newcommand{\bom}{\bm{\omega}_F}
\newcommand{\shockregimelegend}{{\scriptsize Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad By sector (Float): \textcolor{sectorres}{\textbf{---} Res.}\ \textcolor{sectorman}{\textbf{---} Man.}\ \textcolor{sectorserv}{\textbf{---} Serv.}}}

\pgfplotsset{
  slide fig/.style={
    tick label style={font=\tiny},
    label style={font=\tiny},
    title style={font=\tiny, align=center},
    legend style={font=\tiny, draw=none, fill=none},
    grid=both, grid style={dotted, gray!35},
    axis line style={gray!60},
    every axis plot/.append style={line width=1pt},
  }
}


\begin{document}

%--------------------------------------------------------------
\begin{frame}
  \titlepage
  % \vspace{-6pt}
  % {\centering\color{customgreen}\rule{0.35\textwidth}{0.8pt}\par}
  % \begin{center}
  %   \small Building on: Rubbo (2024) \emph{Networks, Phillips Curves, and Monetary Policy}
  % \end{center}
\end{frame}

%==============================================================
\begin{frame}{Outline}
%==============================================================
\vfill
\tableofcontents
\vfill
\end{frame}

%==============================================================
\section{Intro}
%==============================================================
\begin{frame}{Motivation}
%==============================================================

{\footnotesize
\begin{itemize}\setlength{\itemsep}{3pt}
\item Network Phillips curves (Rubbo 2024, La'O--Tahbaz-Salehi 2022): \textbf{closed economy}, unique DC index
\item Open-economy NK (Gal\'i--Monacelli 2005): FX channel, \textbf{no network}
\item \textbf{This paper: both together} --- multi-sector network $+$ FX
\end{itemize}}

\vspace{6pt}
\begin{center}
\begin{tikzpicture}[node distance=0.9cm and 1.3cm, every node/.style={font=\tiny}]
\node[draw, rounded corners, fill=sectorres!25, minimum width=1.7cm] (R) {Resource};
\node[draw, rounded corners, fill=sectorman!25, minimum width=2.0cm, right=of R] (M) {Manufacturing};
\node[draw, rounded corners, fill=sectorserv!25, minimum width=1.7cm, right=of M] (S) {Services};
\draw[-{Latex}] (R) -- (M);
\draw[-{Latex}] (M) -- (S);
\node[above=0.5cm of R, font=\tiny, gray!60!black] (FR) {imports};
\draw[-{Latex}, gray!60] (FR) -- (R);
\node[above=0.5cm of M, font=\tiny, gray!60!black] (FM) {imports};
\draw[-{Latex}, gray!60] (FM) -- (M);
\node[below=0.5cm of R, font=\tiny, customgreen!70!black] (EX) {exports};
\draw[-{Latex}, customgreen!70!black] (R) -- (EX);
\end{tikzpicture}
\end{center}

\vspace{4pt}
{\footnotesize \textbf{Questions:} Optimal FX regime? Does the DC index survive a 2nd cost-push channel?}

\end{frame}

%==============================================================
\begin{frame}{Headline Result}
%==============================================================

\begin{center}
{\tiny \textbf{Total welfare loss ($\times10^4$), full 7-shock model}}\\[2pt]
\begin{tikzpicture}
\begin{axis}[slide fig, xbar, width=7.5cm, height=3.2cm, bar width=10pt,
  symbolic y coords={Peg,Managed,Float}, ytick=data,
  xmin=0, xmax=110, xlabel={\tiny welfare loss ($\times10^4$)}, nodes near coords,
  nodes near coords style={font=\tiny}]
\addplot[fill=customgreen!45!white, draw=customgreen!70!black] coordinates {(25.47,Float) (10.17,Managed) (102.05,Peg)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{2pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{Managed float dominates} both corners
\item \textbf{Peg dominated}: zero autonomy, UIP shock passes straight through
\item \textbf{Driver:} persistence (near-unit-root NFA), not volatility
\end{itemize}}

\vspace{2pt}
{\scriptsize \textit{Units: consumption-equivalent loss, $\times10^{-4}$. Calibrated to Chile's real national input-output table (Banco Central de Chile, Cuadros 12x12, CdeR 2018, ref.\ year 2023) --- full calibration and sources on the next two slides. Solved as the genuine nonlinear model (Dynare), not linearized.}}

\end{frame}

%==============================================================
\begin{frame}{Literature}
%==============================================================

{\scriptsize \textbf{Network side (closed economy):}}
{\tiny
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{1pt}
\item \textbf{Rubbo (2024)} --- Calvo network $\Rightarrow$ unique DC index (1 cost-push channel)
\item \textbf{La'O \& Tahbaz-Salehi (2022)} --- optimal MP in production networks
\item \textbf{Pasten, Schoenle \& Weber (2020)} --- sectoral rigidity heterogeneity drives MP transmission
\end{itemize}}

{\scriptsize \textbf{Open-economy side:}}
{\tiny
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{1pt}
\item \textbf{Gal\'i \& Monacelli (2005)} --- 1-sector SOE: domestic-inflation targeting $\approx$ optimal, peg dominated (ToT channel)
\item \textbf{Fanelli \& Straub (2021)} --- FX intervention as a \emph{second} instrument, separate from $i_t$
\item \textbf{Schmitt-Grohé \& Uribe (2003)} --- debt-elastic premium to stationarize NFA; flags 1st-order welfare risk
\item \textbf{Gopinath \& Itskhoki (2022); Amiti et al.\ (2019); Auer, Levchenko \& Sauré (2019)} --- pass-through, dominant currency, IO spillovers
\item \textbf{Itskhoki \& Mukhin (2023); Bianchi \& Coulibaly (2025); Coulibaly (2023)} --- frontier: managed float/fear-of-floating \emph{optimal}, not behavioral (no network)
\item \textbf{Gnocato, Montes-Gald\'on \& Stamato (2025, ECB); Kalemli-\"Ozcan et al.\ (2025)} --- open economy \emph{with} network (tariffs), fixed Taylor rule, no FX-regime choice\footnote{\tiny Details \& what these papers cite for open-economy production networks: \hyperlink{ecbnetwork}{Appendix}.}
\item \textbf{Qiu, Wang, Xu \& Zanetti (2026, JME)} --- closest paper: derives the open-economy DC index directly extending Rubbo, WIOD 43-country calibration; but \emph{static} (no NFA/persistence), no UIP, and no FX-regime choice --- their own conclusion lists relaxing financial autarky as their top open extension
\end{itemize}}

{\scriptsize \textbf{Gap:} network $+$ open economy $+$ FX \textbf{regime choice} together --- no one has this. Same ranking as Gal\'i--Monacelli but a \textbf{different channel} (risk-premium, not ToT); Managed's dominance echoes Fanelli--Straub \& Itskhoki--Mukhin, but from network persistence.}

\vspace{2pt}
{\tiny $\to$ paper-by-paper comparison to the literature: \hyperlink{littfull}{Appendix}}

\end{frame}

%==============================================================
\begin{frame}{This Paper}
%==============================================================

{\footnotesize
\begin{itemize}\setlength{\itemsep}{4pt}
\item Extend Rubbo $\to$ SOE: imported inputs $\Omega^F$, UIP, debt-elastic premium
\item Ask: optimal FX regime? DC index survive with 2 cost-push channels?
\item \textbf{Three literatures, none overlapping fully} (network only; open economy only; network $+$ open economy but no regime choice) --- \textbf{first to put all three together}
\item \textbf{Not just Gal\'i-Monacelli with sectors}: ranking survives, but the mechanism (risk-premium) and sector heterogeneity are new
\item Calibrate: Chile's real I-O table (Banco Central), also Korea, Czechia \& a stylized baseline; solve \textbf{nonlinear} model directly
\end{itemize}}

\end{frame}

%==============================================================
\section{Model}
%==============================================================
%% my slides

%------------------------------------------------
\begin{frame}{Model Components}

\small

\textbf{Small open economy:} home is a price- and rate-taker in world markets --- foreign price $P_t^{F*}$, foreign rate $i^*$, and foreign demand $D_t^*$ are exogenous; home cannot affect them.

\vspace{0.15cm}

Six building blocks, presented in order:

\vspace{0.15cm}

\begin{enumerate}\setlength\itemsep{4pt}
\item \textbf{Households} --- consumption/labor choice, foreign bond holdings
\item \textbf{Firms: technology} --- Cobb-Douglas production, marginal cost
\item \textbf{Production network} --- Leontief inverse, Domar weights
\item \textbf{Foreign sector} --- law of one price, UIP, net foreign assets
\item \textbf{Equilibrium} --- goods, labor, asset market clearing
\item \textbf{Firms: price setting} --- Calvo pricing, Phillips curve
\item \textbf{Monetary policy} --- float / peg / managed float
\end{enumerate}

\end{frame}


%------------------------------------------------
\begin{frame}{Households}

\small

The representative household maximizes

\[
E_0\sum_{t=0}^{\infty}\beta^t
\left[
\frac{C_t^{1-\gamma}}{1-\gamma}
-
\frac{L_t^{1+\varphi}}{1+\varphi}
\right]
\]

subject to the budget constraint:

\[
P_t^CC_t+B_t+S_tB_t^*=W_tL_t+\Pi_t+(1{+}i_{t-1})B_{t-1}+(1{+}i^*)\big[1-\psi(\hat b^*_{t-1}-\bar b^*)\big]S_tB_{t-1}^*
\]

{\scriptsize $B_t$ (domestic bond) nets to zero in equilibrium --- identical households, no domestic counterparty.}

The consumption aggregator (home bundle, Cobb-Douglas across sectors, vs.\ imports) is:

\[
C_t=
\left[
\omega^{1/\eta}(C_t^H)^{\frac{\eta-1}{\eta}}
+
(1-\omega)^{1/\eta}(C_t^F)^{\frac{\eta-1}{\eta}}
\right]^{\frac{\eta}{\eta-1}},
\qquad
C_t^H=\prod_i(C_{it}^H)^{\beta_i^H}
\]

{\footnotesize where:}

{\scriptsize
\begin{tabular}{@{}llll@{}}
$C_t,L_t$: cons., labor & $\beta,\gamma,\varphi$: disc., RRA, Frisch$^{-1}$ & $P_t^C$: CPI & $W_t$: wage \\
$B_t,B_t^*$: dom./for.\ bond & $i_{t-1},i^*$: dom./for.\ rate & $S_t$: exch.\ rate & $\Pi_t$: profits \\
$\psi,\bar b^*$: debt prem., target & $C_t^H,C_t^F$: home/for.\ goods & $\omega,\eta$: bias, elast.\ & $\beta_i^H$: sector $i$ share \\
\end{tabular}}

\end{frame}


%------------------------------------------------
\begin{frame}{Firms: Technology}

\small

A unit mass of firms $f\in[0,1]$ share the \emph{same} Cobb-Douglas technology, each producing a differentiated variety:

\[
Y_{ift}=A_{it}\,L_{ift}^{\alpha_i}\prod_j X_{ijft}^{\Omega^H_{ij}}\,M_{ift}^{\Omega^F_i}
\]

Varieties aggregate via a CES (Dixit-Stiglitz) index; firm $f$ faces isoelastic demand:

\[
Y_{it}=\Big(\int_0^1 Y_{ift}^{\frac{\varepsilon_i-1}{\varepsilon_i}}df\Big)^{\frac{\varepsilon_i}{\varepsilon_i-1}},
\qquad
Y_{ift}=\Big(\frac{P_{ift}}{P_{it}}\Big)^{-\varepsilon_i}Y_{it}
\]

CRS $\Rightarrow$ marginal cost is \textbf{common across firms} in sector $i$:

\[
MC_{it}=\frac{W_t^{\alpha_i}\prod_j P_{jt}^{\Omega^H_{ij}}\,(S_tP_t^{F*})^{\Omega^F_i}}{A_{it}}
\]

\textbf{Two cost-push channels:} $MC_{it}$ moves with $A_{it}$ (TFP) \emph{and} $S_t$ (import cost). Absent frictions, firms would set $P_{ift}=\frac{\varepsilon_i}{\varepsilon_i-1}MC_{it}$ (markup over cost); with \textbf{Calvo pricing}, only a fraction $\delta_i$ resets each period --- mechanics \& Phillips curve: later slide.

{\scriptsize where:}

{\scriptsize
\begin{tabular}{@{}llll@{}}
$Y_{ift}$: firm $f$ output & $\varepsilon_i$: sector elast.\ & $P_{ift}$: firm $f$ price & $P_{it}$: sector price idx.\ \\
$MC_{it}$: marg.\ cost & $W_t$: wage & $P_{jt}$: sector $j$ price & $S_t P_t^{F*}$: import price \\
\end{tabular}}

\end{frame}


%------------------------------------------------
\begin{frame}{Production Network}

\small

Stacking each firm's input shares $\Omega^H_{ij}$, $\Omega^F_i$ across sectors gives the \textbf{input-output network} $\Omega^H\in\mathbb R^{N\times N}$ and import-exposure vector $\bm\omega_F\in\mathbb R^N$.

\vspace{0.15cm}

The \textbf{Leontief inverse} sums direct and indirect linkages:

\[
(I-\Omega^H)^{-1}=I+\Omega^H+(\Omega^H)^2+\cdots
\]

applied to consumption shares $\bm\beta^H$ (Households) it gives each sector's \textbf{Domar weight} (total GDP sales share); applied to $\bm\omega_F$, its total foreign exposure:

\[
\bm\lambda^\top=(\bm\beta^H)^\top(I-\Omega^H)^{-1},
\qquad
\mathcal M=(I-\Omega^H)^{-1}\bm\omega_F
\]

{\footnotesize where:}

{\footnotesize
\begin{tabular}{@{}llll@{}}
$\Omega^H_{ij}$: dom.\ input share & $\Omega^F_i$: import share & $\bm\omega_F=(\Omega^F_1,\dots,\Omega^F_N)^\top$ & $(I-\Omega^H)^{-1}$: Leontief inverse \\
$\bm\beta^H$: cons.\ shares (Households) & $\bm\lambda$: Domar weights & \multicolumn{2}{l}{$\mathcal M$: total (direct+indirect) foreign exposure} \\
\end{tabular}}

\end{frame}

%------------------------------------------------
\begin{frame}{Firms: Price Setting}

\footnotesize

Calvo pricing through the network delivers the (vector) Phillips curve:

\[
\bm\pi_t=\rho(I-\mathcal V)\mathbb E_t\bm\pi_{t+1}+\mathcal B(\gamma+\varphi)\tilde y_t-\mathcal V\bm\chi_t+\Gamma\Delta e_t
\]

\textbf{Same amplifier, two shocks:} $\mathcal B,\Gamma$ are the \emph{same} rigidity-adjusted operator $\tilde A\equiv\Delta(I-\Omega^H\Delta)^{-1}$, applied to labor share $\bm\alpha$ vs.\ import share $\bm\omega_F$:

\[
\mathcal B=\frac{\tilde A\bm\alpha}{\kappa},
\qquad
\Gamma=\frac{\tilde A\bm\omega_F}{\kappa}
\]

\textbf{Only relative shocks matter:} $\mathcal V\mathbf 1=\mathbf 0$ --- uniform TFP shocks create no cost-push; only sector-specific gaps $\bm\chi_t=(I-\Omega^H)^{-1}\log\mathbf A_t-\bm p_{t-1}$ do.

{\footnotesize where:}

{\scriptsize
\begin{tabular}{@{}llll@{}}
$\bm\pi_t,\tilde y_t$: infl., gap & $\bm\chi_t$: TFP cost-push & $\Delta e_t$: exch.\ rate chg.\ & $\kappa$: GE scalar \\
$\mathcal B,\Gamma$: PC slope, FX pass-thru\ & $\mathcal V$: TFP cost-push matrix & \multicolumn{2}{l}{$\tilde A$: rigidity-adj.\ Leontief} \\
\end{tabular}}

\end{frame}


%------------------------------------------------
\begin{frame}{Foreign Sector}

\footnotesize

$P_t^{F*}$, $i^*$, $D_t^*$ are exogenous AR(1) processes. Law of one price:

\[
P_t^{FH}=S_tP_t^{F*}
\]

UIP, with a debt-elastic risk premium closing the model (Schmitt-Grohé--Uribe) and $RP_t$ the risk-premium shock studied below:

\[
i_t = i^*+\mathbb E_t\Delta e_{t+1}+\psi(\hat b_t^*)+RP_t
\]

NFA accumulates the trade balance; exports respond to relative prices:

\[
\hat b_t^*=\frac{1+i_{t-1}}{\Pi_t^C}\hat b_{t-1}^*+\frac{P_t^HEX_t-P_t^{FH}IM_t}{P_t^C},
\qquad
EX_t=\kappa_{EX}D_t^*\Big(\frac{P_t^H}{P_t^{FH}}\Big)^{-\theta^*}
\]

\textbf{Two channels} to the domestic economy:

\[
\hat s_t+\hat p_t^{F*}\ \rightarrow\ MC_{it}\ \rightarrow\ \pi_{it}
\quad\text{(cost-push)}
\qquad
\hat s_t\ \rightarrow\ EX_t\ \rightarrow\ \tilde y_t\ \rightarrow\ \pi_{it}
\quad\text{(demand)}
\]

{\footnotesize where:}

{\footnotesize
\begin{tabular}{@{}llll@{}}
$P_t^{FH}$: import price & $\hat b_t^*$: NFA (scaled) & $i_{t-1}$: dom.\ rate & $\Pi_t^C$: CPI infl.\ \\
$EX_t,IM_t$: exports, imports & $\kappa_{EX}$: export scale & $D_t^*$: for.\ demand & $\theta^*$: export elast.\ \\
$\hat s_t$: log exch.\ rate & $\hat p_t^{F*}$: log for.\ price & \multicolumn{2}{l}{$\tilde y_t$: output gap} \\
$i^*$: foreign rate (fixed) & $\psi(\cdot)$: debt-elastic wedge & \multicolumn{2}{l}{$RP_t$: risk-premium shock (AR(1))} \\
\end{tabular}}

\end{frame}



%------------------------------------------------
\begin{frame}{Monetary Policy Regimes}

\small

Three exchange-rate regimes are compared. All target the \textbf{DC index $\pi_t^{DC}$, not CPI inflation}: Rubbo (2024) shows CPI targeting does \emph{not} achieve divine coincidence even in the closed economy --- $\pi_t^{DC}$ is the closed-economy-optimal benchmark, so it is the natural rule to test for survival once the economy is opened.

\vspace{0.3cm}

\textbf{Free float}

\[
\hat i_t
=
\phi_\pi\pi_t^{DC}+\phi_y\tilde y_t
\]

The exchange rate adjusts endogenously.

\vspace{0.3cm}

\textbf{Hard peg}

\[
\hat s_t=0
\]

Adjustment occurs through domestic inflation and output.

\vspace{0.3cm}

\textbf{Managed float}

\[
\hat i_t
=
\phi_\pi\pi_t^{DC}+\phi_y\tilde y_t+\phi_s\hat s_t
\]

The central bank partially stabilizes the exchange rate.

\vspace{0.2cm}

{\footnotesize
\begin{tabular}{@{}llll@{}}
$\hat i_t$: policy rate & $\pi_t^{DC}$: DC index & $\tilde y_t$: output gap & $\hat s_t$: exchange rate \\
$\phi_\pi,\phi_y$: inflation, output resp.\ & \multicolumn{2}{l}{$\phi_s$: exchange-rate stabilization weight} \\
\end{tabular}}

\end{frame}


%------------------------------------------------
\begin{frame}{Equilibrium System}

\footnotesize

Everything above collapses to four blocks --- this is what is actually coded up and simulated in Dynare.

\textbf{Phillips curve} (vector, network-amplified, two cost-push channels):

\[
\bm\pi_t=\rho(I-\mathcal V)\mathbb E_t\bm\pi_{t+1}+\mathcal B(\gamma+\varphi)\tilde y_t-\mathcal V\bm\chi_t+\Gamma\Delta e_t
\]

\textbf{IS curve} (aggregate demand; open economy adds the same relative-price term as the cost-push channel above):

\[
\tilde y_t=\mathbb E_t\tilde y_{t+1}-\frac{1}{\gamma}\big(i_t-\mathbb E_t\pi_t^C-r_t^{\mathrm{nat}}\big)-\frac{\eta(1-\omega)}{\gamma}\mathbb E_t\Delta\big(\hat s_t+\hat p_t^{F*}-\hat p_t^H\big)
\]

\textbf{Foreign sector} (UIP closes the FX block; NFA is the only state carried across regimes):

\[
i_t=i^*+\mathbb E_t\Delta e_{t+1}+\psi(\hat b_t^*)+RP_t,
\qquad
\hat b_t^*=\frac{1+i_{t-1}}{\Pi_t^C}\hat b_{t-1}^*+\frac{P_t^HEX_t-P_t^{FH}IM_t}{P_t^C}
\]

\textbf{Policy rule} (regime-dependent; the only block that changes across Float/Peg/Managed --- same rules as previous slide):

\[
\hat i_t=
\begin{cases}
\phi_\pi\pi_t^{DC}+\phi_y\tilde y_t & \text{Float}\\[2pt]
\text{residual (}\hat s_t=0\text{)} & \text{Peg}\\[2pt]
\phi_\pi\pi_t^{DC}+\phi_y\tilde y_t+\phi_s\hat s_t & \text{Managed}
\end{cases}
\]

{\scriptsize $i_t$ above is the level rate (Households, Foreign Sector); $\hat i_t$ is its log-deviation, the Taylor-rule instrument (Monetary Policy Regimes).}

{\scriptsize where:}

{\scriptsize
\begin{tabular}{@{}lll@{}}
$r_t^{\mathrm{nat}}$: natural real rate & $\hat p_t^H$: log home price idx.\ & $\omega,\eta$: home bias, elast.\ \\
$\psi(\cdot),RP_t$: debt wedge, risk-prem.\ & $\pi_t^{DC}$: DC index (target) & $\mathcal B,\Gamma,\mathcal V$: PC coefficients \\
\end{tabular}}

\end{frame}



\begin{frame}{Model Overview}

\large
Small open economy with sectoral production networks.

\vspace{0.3cm}

Foreign variables are exogenous:
\[
P_t^{F*},\qquad i^*,\qquad D_t^*
\]

\vspace{0.3cm}

Model blocks

\begin{enumerate}
\item Households
\item Firms
\item Production network
\item Foreign sector
\item Price setting
\item Monetary policy
\item Equilibrium
\end{enumerate}

\end{frame}

\begin{frame}{Households}

\small

Representative household chooses consumption, labor, and foreign assets.

\vspace{0.2cm}

\[
E_0\sum_{t=0}^{\infty}\beta^t
\left[
\frac{C_t^{1-\gamma}}{1-\gamma}
-
\frac{L_t^{1+\varphi}}{1+\varphi}
\right]
\]

\vspace{0.2cm}

subject to

\[
P_t^CC_t+B_t+S_tB_t^*
=
W_tL_t+\Pi_t
+(1+i_{t-1})B_{t-1}
+(1+i^*)
\left[
1-\psi(\hat b_{t-1}^*-\bar b^*)
\right]
S_tB_{t-1}^*
\]

\vspace{0.25cm}

Consumption basket

\[
C_t=
\left[
\omega^{1/\eta}(C_t^H)^{\frac{\eta-1}{\eta}}
+
(1-\omega)^{1/\eta}(C_t^F)^{\frac{\eta-1}{\eta}}
\right]^{\frac{\eta}{\eta-1}}
\]

\vspace{0.25cm}

Key features

\begin{itemize}
\item Domestic and imported goods are CES substitutes.
\item Domestic goods are Cobb--Douglas across sectors.
\item Foreign bond holdings determine net foreign assets.
\end{itemize}

\end{frame}

\begin{frame}{Firms}

\small

Each sector produces differentiated varieties using labor,
domestic intermediates, and imported inputs.

\vspace{0.2cm}

\[
Y_{ift}
=
A_{it}
L_{ift}^{\alpha_i}
\prod_j
X_{ijft}^{\Omega^H_{ij}}
M_{ift}^{\Omega_i^F}
\]

\vspace{0.25cm}

Marginal cost

\[
MC_{it}
=
\frac{
W_t^{\alpha_i}
\prod_jP_{jt}^{\Omega^H_{ij}}
(S_tP_t^{F*})^{\Omega_i^F}
}
{A_{it}}
\]

\vspace{0.3cm}

Implications

\begin{itemize}
\item Higher wages increase costs.
\item Imported inputs create exchange-rate pass-through.
\item TFP and exchange rates generate two cost-push channels.
\end{itemize}

\end{frame}

\begin{frame}{Production Network}

\small

Input-output linkages amplify shocks across sectors.

\vspace{0.3cm}

Leontief inverse

\[
(I-\Omega^H)^{-1}
=
I+\Omega^H+(\Omega^H)^2+\cdots
\]

\vspace{0.3cm}

Domar weights

\[
\lambda'
=
\beta'(I-\Omega^H)^{-1}
\]

Total foreign exposure

\[
\mathcal M
=
(I-\Omega^H)^{-1}\omega_F
\]

\vspace{0.3cm}

Interpretation

\begin{itemize}
\item Domar weights measure total production importance.
\item Indirect input linkages amplify shocks.
\item Import exposure propagates through the same network.
\end{itemize}

\end{frame}


\begin{frame}{Foreign Sector}

\small

Foreign prices, interest rates, and demand are exogenous.

\vspace{0.25cm}

Law of One Price

\[
P_t^{FH}=S_tP_t^{F*}
\]

\vspace{0.2cm}

UIP

\[
i_t
=
i^*
+
E_t\Delta e_{t+1}
+
\psi(\hat b_t^*)
+
RP_t
\]

\vspace{0.2cm}

Net foreign assets

\[
\hat b_t^*
=
\frac{1+i_{t-1}}{\Pi_t^C}\hat b_{t-1}^*
+
\frac{P_t^HEX_t-P_t^{FH}IM_t}{P_t^C}
\]

\vspace{0.25cm}

Transmission

\begin{itemize}
\item Exchange rate changes import costs.
\item Exchange rate changes export demand.
\item Both channels affect inflation and output.
\end{itemize}

\end{frame}

\begin{frame}{Price Setting}

\small

Calvo pricing delivers a network Phillips curve.

\vspace{0.2cm}

\[
\bm\pi_t
=
\rho(I-\mathcal V)E_t\bm\pi_{t+1}
+
\mathcal B(\gamma+\varphi)\tilde y_t
-
\mathcal V\bm\chi_t
+
\Gamma\Delta e_t
\]

\vspace{0.25cm}

Network coefficients

\[
\mathcal B
=
\frac{\tilde A\alpha}{\kappa},
\qquad
\Gamma
=
\frac{\tilde A\omega_F}{\kappa}
\]

\vspace{0.3cm}

Key insight

\begin{itemize}
\item Labor and import costs propagate through the same network.
\item Exchange-rate shocks become an additional cost-push channel.
\item Only relative TFP shocks affect inflation.
\end{itemize}

\end{frame}

\begin{frame}{Monetary Policy}

\small

The central bank targets the Divine Coincidence (DC) index.

\vspace{0.3cm}

Free float

\[
\hat i_t
=
\phi_\pi\pi_t^{DC}
+
\phi_y\tilde y_t
\]

\vspace{0.25cm}

Hard peg

\[
\hat s_t=0
\]

\vspace{0.25cm}

Managed float

\[
\hat i_t
=
\phi_\pi\pi_t^{DC}
+
\phi_y\tilde y_t
+
\phi_s\hat s_t
\]

\vspace{0.25cm}

The policy rule is the only equation that changes across regimes.

\end{frame}


\begin{frame}{Equilibrium}

\small

The model is solved as a nonlinear DSGE in Dynare.

\vspace{0.2cm}

Four equilibrium blocks

\[
\textbf{Phillips Curve}
\]

\[
\textbf{IS Curve}
\]

\[
\textbf{UIP + NFA}
\]

\[
\textbf{Policy Rule}
\]

\vspace{0.3cm}

Changing the monetary policy block generates the three exchange-rate regimes while all remaining equations stay unchanged.

\end{frame}

%% my slides


\begin{frame}{Model: Overview}
%==============================================================

{\footnotesize\textbf{\textcolor{customgreen!55!black}{Households}}}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item CRRA consumption, disutility of labor; \textbf{Euler eq.}: intertemporal consumption smoothing; labor supply: wage $=$ MRS
\item \textbf{UIP} (derived, not assumed): domestic rate $=$ foreign rate $+$ risk premium $+$ expected depreciation
\end{itemize}}

{\footnotesize\textbf{\textcolor{customgreen!55!black}{Firms}}}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item Cobb-Douglas: labor $+$ domestic network inputs $+$ imports
\item \textbf{Marginal cost} moves with $A_{it}$ (TFP) \textbf{and} $S_t$ (import cost) --- \textbf{two cost-push channels}
\item \textbf{Calvo pricing}: reset prob.\ $\delta_i$, forward-looking reset price; sector-specific stickiness $\to$ heterogeneous slopes
\end{itemize}}

{\footnotesize\textbf{\textcolor{customgreen!55!black}{Open Economy}}}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Law of one price}: import price $=S_t\times$ foreign price; \textbf{UIP} closes the FX block
\item \textbf{NFA law of motion}: current account accumulates foreign bonds; exports: foreign demand $\times$ relative-price elasticity
\end{itemize}}

\vspace{4pt}
{\footnotesize \textbf{Key takeaway:} same network structure as Rubbo (2024), \textbf{plus} an exchange-rate cost-push channel running through imported inputs.}

\vspace{4pt}
{\scriptsize\hyperlink{modeleqns}{$\rightarrow$ full equations \& notation: Appendix}}

\end{frame}

%==============================================================
\begin{frame}{Model: Policy Regimes and Shocks}
%==============================================================

{\footnotesize
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Regime} & \textbf{Rule} & \textbf{Exchange rate} \\
\midrule
Float & $i_t = \bar\imath + \phi_\pi\pi^{DC}_t + \phi_y\tilde y_t$ & fully endogenous (UIP) \\
Peg & $S_t = \bar S$ & fixed; $i_t$ residual \\
Managed & $i_t = \bar\imath + \phi_\pi\pi^{DC}_t + \phi_y\tilde y_t + \phi_s\log(S_t/\bar S)$ & partially stabilized \\
\bottomrule
\end{tabular}}

\vspace{10pt}
{\footnotesize \textbf{Shocks} (AR(1)): $a_{it}$ (TFP, $\rho{=}0.90$)\quad $p^{F*}_t$ (import price, $\rho{=}0.85$)\quad $D^*_t$ (foreign demand, $\rho{=}0.80$)}

\end{frame}

%==============================================================
\begin{frame}{Theory: Regime Comparison --- Predictions}
%==============================================================

{\footnotesize
\begin{tabular}{@{}lccc@{}}
\toprule
 & \textbf{Float} & \textbf{Peg} & \textbf{Managed} \\
\midrule
Exchange rate & endogenous & fixed & partial \\
FX cost-push absorbed by & $S_t$ & $\pi_i,y$ & split \\
Monetary independence & full & none & full \\
\bottomrule
\end{tabular}}

\vspace{10pt}
{\footnotesize \textbf{Naive:} float best. \textbf{Overturned} --- see Welfare section.}

\end{frame}

%==============================================================
\begin{frame}{Does the DC Index Survive a Second Cost-Push Channel?}
%==============================================================

{\footnotesize \textbf{Closed economy (Rubbo):} 1 channel (TFP) $\Rightarrow$ unique DC index}
\vspace{-2pt}
{\scriptsize\[ \bm\phi^\top\mathcal{V}=\bm 0^\top \Rightarrow \pi^{DC}_t = \textstyle\sum_i w^{DC}_i\pi_{it} \]}

\vspace{2pt}
{\footnotesize \textbf{Open economy:} 2 channels (TFP $+$ FX) $\Rightarrow$ need \emph{both}, same weights $\bm\phi$}
\vspace{-2pt}
{\scriptsize\[ \underbrace{\bm\phi^\top\mathcal{V}=\bm 0^\top}_{\text{TFP}} \text{ and } \underbrace{\bm\phi^\top\Gamma = 0}_{\text{FX}} \Rightarrow \textbf{generically infeasible}\]}

\vspace{2pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\topsep}{0pt}
\item \textbf{Answer: no}, generically
\item This calibration: $\cos(\Gamma,\mathcal{B})\approx0.96$ --- close, but not constant
\item \textbf{Fix:} flexible import pricing ($\hat\delta_M{=}1$) restores $\pi^{DC}_t$
\end{itemize}}

\vspace{2pt}
\begin{center}
{\tiny \textbf{$\pi^{DC}_t$, import-price shock}}\\[1pt]
\begin{tikzpicture}
\begin{axis}[slide fig, width=7.5cm, height=2.6cm,
  xlabel={\tiny Quarters}, ylabel={\tiny $\pi^{DC}_t$ (\%)},
  xmin=0.5, xmax=10.5, ymin=-0.03, ymax=0.21, xtick={2,4,6,8,10},
  legend style={font=\tiny, at={(0.97,0.97)}, anchor=north east, row sep=-3pt}]
\addplot[thick, gray!40, dotted] coordinates {(1,0.1928)(2,0.1005)(3,0.0475)(4,0.0166)(5,-0.0014)(6,-0.0116)(7,-0.0172)(8,-0.0198)(9,-0.0208)(10,-0.0207)};
\addlegendentry{Peg}
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.0836)(2,0.0623)(3,0.0451)(4,0.0314)(5,0.0205)(6,0.0120)(7,0.0053)(8,0.0002)(9,-0.0037)(10,-0.0065)};
\addlegendentry{Managed}
\addplot[thick, customgreen!85!black] coordinates {(1,0.0264)(2,0.0222)(3,0.0185)(4,0.0153)(5,0.0126)(6,0.0103)(7,0.0084)(8,0.0068)(9,0.0055)(10,0.0044)};
\addlegendentry{Float}
\end{axis}
\end{tikzpicture}
\end{center}
{\scriptsize Full DC-index algebra, proof, and knife-edge test: Appendix.}

\end{frame}

%==============================================================
\section{Results}
%==============================================================
\begin{frame}{Calibration: Chile, Real IO Data}
%==============================================================
\vspace{-4pt}
{\scriptsize
\renewcommand{\arraystretch}{0.85}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Parameter} & \textbf{Value}\\
\midrule
$\beta$ & 0.99 & $\alpha_1,\alpha_2,\alpha_3$ & 0.503, 0.359, 0.604 \\
$\gamma,\varphi$ & 1.00, 2.00 & $\Omega^F_1,\Omega^F_2,\Omega^F_3$ & 0.077, 0.195, 0.070 \\
$\eta$ & 1.50 & $\beta^H_1,\beta^H_2,\beta^H_3$ & 0.027, 0.229, 0.744 \\
$\varepsilon$ & 8.00 & $\delta_1,\delta_2,\delta_3$ & 0.90, 0.31, 0.16 \\
$\theta^*$ & 2.00 & $\psi$ & 0.020 \\
$\beta^{F,tot}$ & 0.10 & $\phi_\pi,\phi_y,\phi_s$ & 1.50, 0.50, 0.30 \\
\bottomrule
\end{tabular}}

\vspace{-2pt}
{\scriptsize Dense $\Omega^H$ (order: Res., Man., Serv.), all 9 entries used, no triangular assumption:
\vspace{-4pt}
\[
\Omega^H=\begin{pmatrix}0.075&0.153&0.193\\0.099&0.202&0.145\\0.002&0.058&0.266\end{pmatrix},
\quad \text{Export share} = (0.602,\,0.180,\,0.036)
\]}

\vspace{-8pt}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item Resource = mining/copper (Chile's export engine, 60\% of own output exported) $\to$ Manuf.\ (mid-chain) $\to$ Services (largest value added, sticky)\footnote{\tiny Sector concordance (Banco Central de Chile, CdeR 2018, 12 activities $\to$ our 3): \textbf{Resource} = \{1 Agriculture, forestry \& fishing, 2 Mining\}; \textbf{Manufacturing} = \{3 Manufacturing industry, 4 Electricity, gas, water \& waste management, 5 Construction\}; \textbf{Services} = \{6 Trade, hotels \& restaurants, 7 Transport, communications \& information services, 8 Financial intermediation, 9 Real estate \& housing services, 10 Business services, 11 Personal services, 12 Public administration\}. Import \& export data exist at the sector level from the same tables: imports $=$ imported intermediate input use per buying sector (sheet 21, underlies $\Omega^F_i$); exports $=$ exports of each sector's own output (sheet 20, underlies the export-share figures above).}
\item $\alpha_i,\Omega^H_{ij},\Omega^F_i,\beta^H_i$: \textbf{data} (Banco Central de Chile, Cuadros 12x12, CdeR 2018, ref.\ 2023)
\item $\delta_1,\delta_2,\delta_3$: \textbf{literature} --- euro-area price-change frequencies (Dhyne et al.\ 2005/ECB IPN) $\to$ quarterly reset prob.; cross-checked vs.\ Nakamura \& Steinsson (2008) US PPI
\item $\beta,\gamma,\varphi,\eta,\varepsilon,\theta^*,\psi,\phi_\pi,\phi_y,\phi_s$: literature-standard. Solved nonlinear, order 1
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Calibration: Network Properties}
%==============================================================

{\scriptsize
$\lambda_{D,i} = \big[(\bm\beta^H)^\top(I-\Omega^H)^{-1}\big]_i,\quad
\mathcal{M}_i = \big[(I-\Omega^H)^{-1}\Omega^F\mathbf{1}\big]_i,\quad
w^{DC}_i \propto \lambda_{D,i}\,\tfrac{1-\hat\delta_i}{\hat\delta_i}$}

{\tiny $\lambda_{D,i}$: Domar weight\quad $\mathcal{M}_i$: import centrality\quad $w^{DC}_i$: DC weight}

\vspace{2pt}
\begin{center}
{\tiny \textbf{$\lambda_{D,i}$ vs.\ $\mathcal{M}_i$ vs.\ export share vs.\ $w^{DC}_i$}}\\[1pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar, bar width=4.5pt, width=8cm, height=2.8cm,
  symbolic x coords={Resource,Manuf.,Services}, xtick=data,
  ymin=0, ymax=1.15, ylabel={\tiny value},
  legend style={font=\tiny, at={(0.5,1.22)}, anchor=north, legend columns=4}]
\addplot[fill=customgreen!55, draw=customgreen!70!black] coordinates {(Resource,0.072) (Manuf.,0.382) (Services,1.108)};
\addlegendentry{$\lambda_{D,i}$}
\addplot[fill=gray!45, draw=gray!70!black] coordinates {(Resource,0.155) (Manuf.,0.285) (Services,0.119)};
\addlegendentry{$\mathcal{M}_i$}
\addplot[fill=sectorman!70!white, draw=sectorman] coordinates {(Resource,0.602) (Manuf.,0.180) (Services,0.036)};
\addlegendentry{Export share}
\addplot[fill=riskred!55!white, draw=riskred!80!black] coordinates {(Resource,0.0002) (Manuf.,0.072) (Services,0.928)};
\addlegendentry{$w^{DC}_i$}
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{2pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\topsep}{0pt}
\item FX exposure runs \textbf{opposite} to Domar weight
\item Resource: small weight (0.07), most FX-exposed ($\mathcal{M}_i=0.16$) --- and \emph{is} the export engine (60\% of own output exported, copper)
\item Services: dominates output ($\lambda_D=1.11$), FX-exposed only \textbf{indirectly}; $w^{DC}_i$ is essentially a Services index (0.93)
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{IRFs: Foreign Risk-Premium Shock --- the Key Mechanism}
%==============================================================
\vspace{-8pt}
{\scriptsize \textbf{The shock that decides the ranking:} 75\% of Peg's welfare loss (Chile calibration).}
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.4406, ymax=2.6632, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,1.9362)(2,1.3931)(3,0.9778)(4,0.6622)(5,0.4248)(6,0.2482)(7,0.1188)(8,0.0259)(9,-0.0389)(10,-0.0825)};
\addplot[thick, customgreen!85!black] coordinates {(1,2.3051)(2,1.7049)(3,1.2345)(4,0.8680)(5,0.5846)(6,0.3672)(7,0.2021)(8,0.0785)(9,-0.0126)(10,-0.0782)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: risk premium $RP_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.0044, ymax=1.1299, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.8000)(3,0.6400)(4,0.5120)(5,0.4096)(6,0.3277)(7,0.2621)(8,0.2097)(9,0.1678)(10,0.1342)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-6.1281, ymax=1.4962, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-5.2484)(2,-3.1995)(3,-1.8431)(4,-0.9460)(5,-0.3652)(6,-0.0021)(7,0.2127)(8,0.3280)(9,0.3778)(10,0.3859)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.2958)(2,-0.1806)(3,-0.0963)(4,-0.0351)(5,0.0084)(6,0.0382)(7,0.0577)(8,0.0694)(9,0.0752)(10,0.0767)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.6165)(2,0.4686)(3,0.3545)(4,0.2662)(5,0.1979)(6,0.1451)(7,0.1044)(8,0.0732)(9,0.0493)(10,0.0311)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.3523, ymax=0.4122, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-0.2641)(2,-0.0654)(3,0.0119)(4,0.0505)(5,0.0682)(6,0.0733)(7,0.0709)(8,0.0643)(9,0.0557)(10,0.0464)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.2229)(2,-0.0530)(3,-0.0344)(4,-0.0211)(5,-0.0115)(6,-0.0046)(7,0.0001)(8,0.0031)(9,0.0050)(10,0.0059)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.3240)(2,-0.0347)(3,-0.0320)(4,-0.0284)(5,-0.0246)(6,-0.0211)(7,-0.0178)(8,-0.0148)(9,-0.0123)(10,-0.0101)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-4.5042, ymax=1.0944, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.1169)(2,-0.0469)(3,-0.0200)(4,-0.0059)(5,0.0016)(6,0.0052)(7,0.0065)(8,0.0066)(9,0.0059)(10,0.0050)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-1.2732)(2,-0.7283)(3,-0.3839)(4,-0.1702)(5,-0.0419)(6,0.0314)(7,0.0696)(8,0.0862)(9,0.0898)(10,0.0861)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,-3.8582)(2,-2.4243)(3,-1.4392)(4,-0.7698)(5,-0.3250)(6,-0.0387)(7,0.1366)(8,0.2352)(9,0.2821)(10,0.2948)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0063)(2,0.0060)(3,0.0042)(4,0.0027)(5,0.0016)(6,0.0009)(7,0.0004)(8,0.0000)(9,-0.0002)(10,-0.0004)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.0779)(2,-0.0562)(3,-0.0379)(4,-0.0228)(5,-0.0108)(6,-0.0013)(7,0.0059)(8,0.0113)(9,0.0152)(10,0.0179)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.2242)(2,-0.1304)(3,-0.0626)(4,-0.0150)(5,0.0175)(6,0.0387)(7,0.0514)(8,0.0580)(9,0.0602)(10,0.0593)};
\addplot[thick, sectorres] coordinates {(1,0.0284)(2,0.0173)(3,0.0107)(4,0.0063)(5,0.0033)(6,0.0013)(7,0.0000)(8,-0.0008)(9,-0.0012)(10,-0.0015)};
\addplot[thick, sectorman] coordinates {(1,0.1397)(2,0.0860)(3,0.0514)(4,0.0298)(5,0.0169)(6,0.0096)(7,0.0060)(8,0.0047)(9,0.0046)(10,0.0053)};
\addplot[thick, sectorserv] coordinates {(1,0.4484)(2,0.3652)(3,0.2924)(4,0.2302)(5,0.1777)(6,0.1342)(7,0.0984)(8,0.0693)(9,0.0459)(10,0.0273)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-3.1698, ymax=1.1945, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-2.6662)(2,0.5680)(3,0.6909)(4,0.5401)(5,0.3998)(6,0.2906)(7,0.2077)(8,0.1451)(9,0.0983)(10,0.0633)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.4197)(2,-0.1464)(3,0.0092)(4,0.0887)(5,0.1217)(6,0.1276)(7,0.1187)(8,0.1027)(9,0.0843)(10,0.0660)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,-0.1771)(2,-0.0746)(3,-0.0093)(4,0.0301)(5,0.0519)(6,0.0617)(7,0.0638)(8,0.0609)(9,0.0552)(10,0.0480)};
\addplot[thick, sectorres, dashed] coordinates {(1,-0.2153)(2,0.0668)(3,0.0795)(4,0.0667)(5,0.0537)(6,0.0428)(7,0.0338)(8,0.0263)(9,0.0202)(10,0.0151)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0299)(2,0.0222)(3,0.0176)(4,0.0146)(5,0.0125)(6,0.0109)(7,0.0095)(8,0.0084)(9,0.0073)(10,0.0062)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.0044)(2,0.0061)(3,0.0128)(4,0.0167)(5,0.0187)(6,0.0192)(7,0.0188)(8,0.0176)(9,0.0160)(10,0.0142)};
\addplot[thick, sectorres] coordinates {(1,0.2534)(2,0.0276)(3,-0.0037)(4,-0.0111)(5,-0.0138)(6,-0.0147)(7,-0.0146)(8,-0.0139)(9,-0.0128)(10,-0.0117)};
\addplot[thick, sectorman] coordinates {(1,0.1306)(2,0.0720)(3,0.0337)(4,0.0096)(5,-0.0049)(6,-0.0130)(7,-0.0169)(8,-0.0181)(9,-0.0177)(10,-0.0164)};
\addplot[thick, sectorserv] coordinates {(1,0.0365)(2,0.0300)(3,0.0241)(4,0.0190)(5,0.0146)(6,0.0109)(7,0.0078)(8,0.0053)(9,0.0033)(10,0.0017)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}

%==============================================================
\begin{frame}{Network Exposure \& Sector Welfare Preferences}
%==============================================================

\vspace{-6pt}
\begin{center}
{\tiny \textbf{$\lambda_{D,i}$ vs.\ $\mathcal{M}_i$ vs.\ export share:} \textcolor{customgreen!55!black}{\textbf{$\blacksquare$} $\lambda_{D,i}$}\ \textcolor{gray!60!black}{\textbf{$\blacksquare$} $\mathcal{M}_i$}\ \textcolor{sectorman}{\textbf{$\blacksquare$} Export share}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar, bar width=7pt, width=8.5cm, height=2.0cm,
  symbolic x coords={Resource,Manuf.,Services}, xtick=data,
  ymin=0, ymax=1.15, ylabel={\tiny value}]
\addplot[fill=customgreen!55, draw=customgreen!70!black] coordinates {(Resource,0.072) (Manuf.,0.382) (Services,1.108)};
\addplot[fill=gray!45, draw=gray!70!black] coordinates {(Resource,0.155) (Manuf.,0.285) (Services,0.119)};
\addplot[fill=sectorman!70!white, draw=sectorman] coordinates {(Resource,0.602) (Manuf.,0.180) (Services,0.036)};
\end{axis}
\end{tikzpicture}

\vspace{0pt}
{\tiny \textbf{Own dispersion cost by regime ($\times10^4$):} \textcolor{customgreen!85!black}{\textbf{$\blacksquare$} Float}\ \textcolor{gray!55}{\textbf{$\blacksquare$} Managed}\ \textcolor{riskred!70!black}{\textbf{$\blacksquare$} Peg}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar, width=8.5cm, height=2.0cm, bar width=7pt,
  symbolic x coords={Resource,Manuf.,Services}, xtick=data,
  ymin=0, ymax=24, ylabel={\tiny cost ($\times10^4$)}]
\addplot[fill=customgreen!85!black] coordinates {(Resource,0.04) (Manuf.,3.30) (Services,20.73)};
\addplot[fill=gray!55] coordinates {(Resource,0.04) (Manuf.,1.82) (Services,6.70)};
\addplot[fill=riskred!70!white] coordinates {(Resource,0.37) (Manuf.,5.36) (Services,16.79)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\scriptsize \textbf{Exposure} $\to$ \textbf{pass-through} $\Gamma_i=\tfrac{[\Delta(I-\Omega\Delta)^{-1}\bm\omega_F]_i}{\kappa_i}$, \textbf{welfare weight} $\kappa_i=\lambda_{D,i}\varepsilon\tfrac{1-\hat\delta_i}{\hat\delta_i}$:}

\vspace{-1pt}
\begin{center}
{\tiny
\begin{tabular}{lcccc}
\toprule
 & $\mathcal{M}_i$ & Export share & $\Gamma_i$ & $\kappa_i$ \\
\midrule
Resource & \textbf{0.155} & \textbf{0.602} & \textbf{1.091} & 0.07 \\
Manuf.\ & 0.285 & 0.180 & 0.001 & 21.5 \\
Services & 0.119 & 0.036 & $<$0.0001 & \textbf{276.3} \\
\bottomrule
\end{tabular}}
\end{center}

\vspace{1pt}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item Under literature-sourced Calvo stickiness ($\hat\delta_3=0.03$), $\kappa_i$ concentrates in Services --- FX pass-through barely matters for Services' welfare cost \emph{relative to its own weight}: almost all of $\Gamma_i$ funnels through Resource
\item \textbf{Managed dominates every sector.} Peg costliest for Resource (0.37) \& Manuf.\ (5.36); for Services, \textbf{Float is costliest} (20.73 $>$ Peg's 16.79) --- near-unit-root $S_t$ under Float feeds the stickiest sector's marginal cost for a long time
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Welfare: Headline \& Persistence}
%==============================================================

\vspace{-6pt}
\begin{center}
{\tiny \textbf{Total welfare loss ($\times10^4$)}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, xbar, width=8.5cm, height=1.9cm, bar width=8pt,
  symbolic y coords={Peg,Managed,Float}, ytick=data,
  xmin=0, xmax=110, xlabel={\tiny welfare loss ($\times10^4$)}, nodes near coords,
  nodes near coords style={font=\tiny}]
\addplot[fill=customgreen!45!white, draw=customgreen!70!black] coordinates {(25.47,Float) (10.17,Managed) (102.05,Peg)};
\end{axis}
\end{tikzpicture}

\vspace{0pt}
{\tiny \textbf{Output gap vs.\ sector dispersion:} \textcolor{gray!55}{\textbf{$\blacksquare$} Output gap}\ \textcolor{customgreen!25!black}{\textbf{$\blacksquare$} Disp.\ Res.}\ \textcolor{customgreen!55!black}{\textbf{$\blacksquare$} Disp.\ Man.}\ \textcolor{customgreen!85!black}{\textbf{$\blacksquare$} Disp.\ Serv.}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar stacked, width=8.5cm, height=2.0cm, bar width=13pt,
  symbolic x coords={Float,Managed,Peg}, xtick=data,
  ymin=0, ymax=110, ylabel={\tiny welfare loss ($\times10^4$)}]
\addplot[fill=gray!35] coordinates {(Float,1.41) (Managed,1.61) (Peg,79.53)};
\addplot[fill=customgreen!25] coordinates {(Float,0.04) (Managed,0.04) (Peg,0.37)};
\addplot[fill=customgreen!55] coordinates {(Float,3.30) (Managed,1.82) (Peg,5.36)};
\addplot[fill=customgreen!85!black] coordinates {(Float,20.73) (Managed,6.70) (Peg,16.79)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\scriptsize $\mathcal{W}=\tfrac12\big[(\gamma+\varphi)\mathrm{Var}(\tilde y_t)+\sum_i\lambda_{D,i}\varepsilon\tfrac{1-\hat\delta_i}{\hat\delta_i}\mathrm{Var}(\pi_{it})\big]$ --- \textbf{Peg dominated} on output gap \& dispersion, every sector.}

\vspace{2pt}
{\scriptsize \textbf{Why: persistence, not volatility.}
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item Float's $S_t$ (near-unit-root NFA) feeds \emph{every} sector's MC, even for domestic shocks
\item Peg's output-gap term explodes (79.5) because it has zero monetary autonomy to absorb the risk-premium shock
\item Slow decay, not larger impact, drives Float's Services dispersion above Managed's
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Shock Decomposition of Welfare Cost}
%==============================================================

\begin{center}
{\tiny \textbf{Welfare loss by regime, by shock source ($\times10^4$), full 7-shock model}}\\[2pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar stacked, width=9.2cm, height=3.7cm, bar width=26pt,
  symbolic x coords={Float,Managed,Peg}, xtick=data,
  ymin=0, ymax=110, ylabel={\tiny welfare loss ($\times10^4$)},
  legend style={font=\tiny, at={(0.5,1.28)}, anchor=north, legend columns=3}]
\addplot[fill=gray!45, draw=gray!70!black] coordinates {(Float,23.14) (Managed,8.02) (Peg,11.31)};
\addlegendentry{TFP}
\addplot[fill=customgreen!70!black, draw=customgreen!85!black] coordinates {(Float,0.07) (Managed,1.05) (Peg,10.09)};
\addlegendentry{Import price (FX)}
\addplot[fill=sectorman!80!white, draw=sectorman] coordinates {(Float,0.05) (Managed,0.13) (Peg,1.67)};
\addlegendentry{Foreign demand}
\addplot[fill=sectorserv!60!white, draw=sectorserv] coordinates {(Float,0.11) (Managed,0.18) (Peg,2.16)};
\addlegendentry{Export price (ToT)}
\addplot[fill=riskred!70!white, draw=riskred!90!black] coordinates {(Float,2.11) (Managed,0.79) (Peg,76.83)};
\addlegendentry{Risk premium (UIP)}
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{4pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{3pt}
\item \textbf{Risk-premium/UIP dominates Peg} (75\% of its total)
\item \textbf{Ranking:} Managed $<$ Float $\ll$ Peg --- Peg no longer competitive
\end{itemize}}

{\scriptsize \textit{2nd-order check: this is the channel a genuine order-2 solution could move most --- it does move most, but only $+0.7\%$ (see Conclusion \& appendix).}}

\end{frame}

%==============================================================
\begin{frame}{Mechanisms: Why Each Regime Costs What It Costs}
%==============================================================

{\scriptsize
\begin{tabular}{@{}p{1.2cm}p{4.3cm}p{4.3cm}@{}}
\toprule
\textbf{Regime} & \textbf{Cost channel} & \textbf{What sets the magnitude} \\
\midrule
\textbf{Float} & $S_t$'s near-unit-root persistence (NFA state) feeds $\Gamma_i\Delta e_t$ into \emph{every} sector's MC, even for shocks with no FX content --- pure dispersion cost & Persistence of debt-elastic premium ($\psi$); $\Gamma_i$ (import exposure); $\kappa_i$ of the most central/sticky sector (Services: $20.7$ of $25.5$) \\
\textbf{Peg} & Zero monetary autonomy: output-gap term explodes ($79.5$ of $102.1$, $\approx$78\%) on the UIP shock; $\Delta e_t\equiv0$ \emph{relocates}, not removes, adjustment into upstream $P^H$ & Absence of $\phi_\pi,\phi_y$; real vs.\ nominal share of the shock; upstream sector's Domar weight \\
\textbf{Managed} & $\phi_S\Delta\log S_t$ damps $\Gamma_i\Delta e_t$ without fully re-imposing peg's relocation cost --- dominates every sector at once & Interior optimal $\phi_S^*\approx0.20$: too low $\to$ UIP shock leaks in; too high $\to$ autonomy lost \\
\bottomrule
\end{tabular}}

\vspace{1pt}
{\scriptsize \textbf{Two forces, always in tension:} $\Gamma$-channel favors peg (switches off $\Gamma_i\Delta e_t$); \textbf{relocation} channel favors float (peg's fixed $e_t$ forces the same adjustment into upstream prices). Which wins depends on $\Gamma_i$ vs.\ $\kappa_i$; managed is the only regime improving on \emph{both} corners for every sector.}

\vspace{1pt}
{\scriptsize \textbf{Analytical or simulated? Both.}
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item \textbf{Channels --- closed-form:} $\Gamma_i=[\tilde A\bm\omega_F]_i/\kappa$ (pass-through), $\kappa_i=\lambda_{D,i}\varepsilon\tfrac{1-\hat\delta_i}{\hat\delta_i}$ (welfare weight), DC-survival $\bm\phi^\top\mathcal V{=}\bm0^\top$ \emph{and} $\bm\phi^\top\Gamma{=}0$ --- from primitives alone, no model solve.
\item \textbf{Magnitudes/ranking --- need simulation:} $\mathrm{Var}(\pi_{it}),\mathrm{Var}(\tilde y_t)$ come from the full linear RE solution (order-1 Dynare) --- no closed form once $\phi_S{>}0$ or $\Omega^H$ dense. Managed$<$Float$\ll$Peg and $\phi_S^*{\approx}0.20$ are simulated, but robust across all four calibrations.
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Robustness: South Korea, Czechia \& the Stylized Baseline}
%==============================================================

\vspace{-4pt}
\begin{center}
{\tiny \textbf{Total welfare loss ($\times10^4$)}}\\[1pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar, width=8.5cm, height=2.6cm, bar width=7pt,
  symbolic x coords={Float,Managed,Peg}, xtick=data,
  ymin=0, ymax=145, ylabel={\tiny welfare loss ($\times10^4$)}, nodes near coords,
  nodes near coords style={font=\tiny, rotate=90, anchor=west},
  legend style={font=\tiny, at={(0.5,1.28)}, anchor=north, legend columns=4}]
\addplot[fill=customgreen!45!white, draw=customgreen!70!black] coordinates {(Float,25.47) (Managed,10.17) (Peg,102.05)};
\addlegendentry{Chile}
\addplot[fill=sectorman!55!white, draw=sectorman!80!black] coordinates {(Float,38.91) (Managed,14.43) (Peg,133.81)};
\addlegendentry{S.\ Korea}
\addplot[fill=riskred!45!white, draw=riskred!80!black] coordinates {(Float,32.20) (Managed,11.82) (Peg,84.03)};
\addlegendentry{Czechia}
\addplot[fill=gray!35!white, draw=gray!70!black] coordinates {(Float,13.64) (Managed,5.36) (Peg,49.63)};
\addlegendentry{Stylized}
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}\setlength{\topsep}{0pt}
\item \textbf{South Korea}: diversified manufacturing exporter (Bank of Korea, 2022), deeper manufacturing import intensity than Chile ($\Omega^F_{\text{manuf}}=0.24$ vs.\ $0.19$)
\item \textbf{Czechia}: third real IO calibration (Eurostat, 2022), deepest manufacturing import intensity ($\Omega^F_{\text{manuf}}=0.32$, EU/German supply chains)
\item \textbf{Stylized}: original hand-picked triangular $\Omega^H$ (Rubbo-style, no data) --- checks the result isn't an artifact of network shape
\item \textbf{Ranking survives in all four:} Managed $<$ Float $\ll$ Peg
\end{itemize}}

\vspace{1pt}
{\scriptsize \textbf{UIP dominates Peg's loss in all four:} 75\% Chile, 72\% Korea, 65\% Czechia, 81\% stylized --- not specific to one calibration. All solved nonlinear (Dynare), not linearized.}

\end{frame}

%==============================================================
\section{Conclusion}
%==============================================================
\begin{frame}{Generalization: Optimal FX Stabilization $\phi_s$}
%==============================================================

\vspace{-4pt}
\begin{center}
{\tiny \textbf{Welfare loss vs.\ $\phi_s$ ($\times10^4$), full 7-shock model, stylized network}}\\[1pt]
\begin{tikzpicture}
\begin{axis}[slide fig, width=8.5cm, height=3.2cm,
  xlabel={\tiny $\phi_s$}, ylabel={\tiny welfare loss ($\times10^4$)},
  xmin=-0.05, xmax=2.05, ymin=3.5, ymax=23.0]
\addplot[thick, customgreen!85!black, mark=*, mark size=1.3pt] coordinates {(0.00,13.639)(0.10,5.259)(0.20,4.744)(0.30,5.357)(0.50,7.520)(0.75,10.529)(1.00,13.353)(1.50,18.128)(2.00,21.878)};
\node[font=\tiny] at (axis cs:0.20,4.0) {min $\approx0.20$};
\draw[dashed, gray!50, thin] (axis cs:0.30,3.5) -- (axis cs:0.30,5.357);
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Why interior:} too low $\to$ UIP shock leaks in; too high $\to$ loses autonomy
\item $\phi_s{=}0\to0.20$: loss $13.64\to4.74$. \textbf{Peaked, not flat:} $0.20$ beats calibrated $0.30$ by $\approx$13\%
\item Beyond $\approx0.2$: rises fast, back above Float by $\phi_s{=}1$
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Generalization: Robustness}
%==============================================================
\vspace{-4pt}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Welfare vs.\ import intensity $\kappa$}, title style={yshift=-3pt, font=\tiny}, width=8.5cm, height=2.5cm,
  xlabel={\tiny scale $\kappa$}, ylabel={\tiny welfare loss ($\times10^4$)},
  xmin=0.1, xmax=2.6, ymin=3.5, ymax=72.0,
  legend style={font=\tiny, at={(0.97,0.97)}, anchor=north east, row sep=-3pt}]
\addplot[thick, gray!45, densely dotted, mark=*, mark size=1.1pt] coordinates {(0.25,68.0923) (0.50,60.6833) (0.75,54.6193) (1.00,49.6315) (1.50,42.1035) (2.00,36.9361) (2.50,33.4123)};
\addlegendentry{Peg}
\addplot[thick, gray!60!black, dashed, mark=*, mark size=1.1pt] coordinates {(0.25,7.4890) (0.50,6.4723) (0.75,5.7864) (1.00,5.3573) (1.50,5.0557) (2.00,5.2553) (2.50,5.7752)};
\addlegendentry{Managed}
\addplot[thick, customgreen!85!black, mark=*, mark size=1.1pt] coordinates {(0.25,11.3694) (0.50,11.8072) (0.75,12.5905) (1.00,13.6393) (1.50,16.3097) (2.00,19.5097) (2.50,23.0897)};
\addlegendentry{Float}
\end{axis}
\end{tikzpicture}

\vspace{1pt}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Welfare vs.\ heterogeneity $\theta$}, title style={yshift=-3pt, font=\tiny}, width=8.5cm, height=2.5cm,
  xlabel={\tiny heterogeneity $\theta$}, ylabel={\tiny welfare loss ($\times10^4$)},
  xmin=-0.05, xmax=1.55, ymin=4.0, ymax=64.0]
\addplot[thick, gray!45, densely dotted, mark=*, mark size=1.1pt] coordinates {(0.00,36.7987) (0.25,39.3696) (0.50,42.3178) (0.75,45.7107) (1.00,49.6315) (1.25,54.1842) (1.50,59.4992)};
\addplot[thick, gray!60!black, dashed, mark=*, mark size=1.1pt] coordinates {(0.00,4.8775) (0.25,4.9072) (0.50,4.9898) (0.75,5.1356) (1.00,5.3573) (1.25,5.6708) (1.50,6.0956)};
\addplot[thick, customgreen!85!black, mark=*, mark size=1.1pt] coordinates {(0.00,16.2751) (0.25,15.5831) (0.50,14.9098) (0.75,14.2598) (1.00,13.6393) (1.25,13.0564) (1.50,12.5219)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\footnotesize \textbf{Managed $<$ Float $<$ Peg robust} (stylized network, as in the FX-stabilization sweep), but the gap moves in \textbf{opposite directions}:
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item $\kappa$ (import scale): Peg falls, Float rises --- \textbf{converge}
\item $\theta$ (concentration): Peg rises, Float falls --- \textbf{diverge}
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Isolating the Network Channel}
%==============================================================

{\scriptsize \textbf{New sweep, stylized triangular network} (not Chile's dense $\Omega^H$): scale domestic I-O links $OH_{21},OH_{32}$ by $\rho$. $\rho{=}0$: no domestic network. $\rho{=}1$: baseline.}

\vspace{-10pt}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Welfare vs.\ network density $\rho$}, title style={yshift=-3pt, font=\tiny}, width=8.5cm, height=2.15cm,
  xlabel={\tiny density $\rho$ (0 = no network)}, ylabel={\tiny welfare loss ($\times10^4$)},
  xmin=-0.1, xmax=3.1, ymin=0, ymax=88,
  legend style={font=\tiny, at={(0.03,0.97)}, anchor=north west, row sep=-3pt}]
\addplot[thick, gray!45, densely dotted, mark=*, mark size=1.1pt] coordinates {(0.00,39.3342) (0.25,41.4210) (0.50,43.8192) (0.75,46.5537) (1.00,49.6315) (1.50,56.7317) (2.00,64.8207) (3.00,82.8659)};
\addlegendentry{Peg}
\addplot[thick, gray!60!black, dashed, mark=*, mark size=1.1pt] coordinates {(0.00,5.0105) (0.25,4.9444) (0.50,4.9564) (0.75,5.0824) (1.00,5.3573) (1.50,6.4928) (2.00,8.6482) (3.00,17.3960)};
\addlegendentry{Managed}
\addplot[thick, customgreen!85!black, mark=*, mark size=1.1pt] coordinates {(0.00,6.6630) (0.25,7.3884) (0.50,8.7522) (0.75,10.8103) (1.00,13.6393) (1.50,21.9834) (2.00,34.5886) (3.00,75.8864)};
\addlegendentry{Float}
\draw[dashed, gray!50, thin] (axis cs:1.00,0) -- (axis cs:1.00,49.6315);
\node[font=\tiny, gray!60!black] at (axis cs:1.35,3) {baseline};
\end{axis}
\end{tikzpicture}

\vspace{-4pt}
{\tiny \textbf{Network welfare premium by sector ($\times10^4$)}, total$(\rho{=}1){-}$total$(\rho{=}0)$: \textcolor{sectorres}{\textbf{$\blacksquare$} Resource}\ \textcolor{sectorman}{\textbf{$\blacksquare$} Manuf.}\ \textcolor{sectorserv}{\textbf{$\blacksquare$} Services}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, ybar, width=8.5cm, height=1.9cm, bar width=6pt,
  symbolic x coords={Float,Managed,Peg}, xtick=data,
  ymin=-9, ymax=6.5, ylabel={\tiny premium ($\times10^4$)}]
\addplot[fill=sectorres!70!white, draw=sectorres] coordinates {(Float,0.2003) (Managed,0.0953) (Peg,0.9964)};
\addplot[fill=sectorman!70!white, draw=sectorman] coordinates {(Float,1.3252) (Managed,0.5314) (Peg,4.1825)};
\addplot[fill=sectorserv!70!white, draw=sectorserv] coordinates {(Float,5.1359) (Managed,-0.7296) (Peg,-7.5628)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{-2pt}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{-1pt}\setlength{\topsep}{0pt}
\item \textbf{Managed is almost network-neutral:} $\rho{=}0\to1$ adds only $+0.35$ to Managed vs.\ $+6.98$ Float, $+10.30$ Peg
\item \textbf{Denser network $\Rightarrow$ corners diverge from Managed:} by $\rho{=}3$, Float ($75.9$) catches Peg ($82.9$), both $\gg$ Managed ($17.4$)
\item \textbf{Burden lands on different sectors:} Peg/Managed hit upstream Resource/Manuf., Services \emph{gains}; Float hits Services most
\item \textbf{Verdict:} network \textbf{triples} the regime gap as density rises ($\rho{:}1\to3$) and \textbf{redirects} who bears the cost
\end{itemize}}

\end{frame}

%==============================================================
\begin{frame}{Conclusion}
%==============================================================

{\footnotesize
\begin{itemize}\setlength{\itemsep}{5pt}
\item \textbf{Managed float optimal}: 10.17 vs.\ Float 25.47, Peg 102.05 ($\times10^4$, Chile calibration)
\item \textbf{Driver:} risk-premium/UIP dominates Peg (75\%); TFP persistence dominates Float \& Managed
\item \textbf{Optimal $\phi_s\approx0.20$}: sharply peaked --- over-stabilizing $S_t$ is costly
\item \textbf{Ranking robust} to $\kappa,\theta,\rho$ (network density) \& to Korea/Czechia calibrations; the \emph{gap} is not
\item \textbf{Network channel isolated} (stylized network, $\rho$ dial): $\rho{=}0\to1$ adds only $+0.35$ to Managed vs.\ $+6.98$ Float, $+10.30$ Peg --- \textbf{denser networks favor Managed most}, and reallocate the network's sectoral burden (Services gains under Peg, loses under Float)
\item \textbf{Vs.\ literature:} same ranking as Gal\'i--Monacelli, different channel (risk-premium, not ToT)
\item \textbf{2nd-order-verified} (Dynare order=2, pruned, simulated): loss rises $\le2\%$ net of MC noise, ranking unchanged --- see appendix
\item \textbf{Next:} many-sector OECD TiVA calibration (beyond the 3-sector Chile/Korea/Czechia data)
\end{itemize}}

\end{frame}

%==============================================================
\appendix
\section*{Appendix: Second-Order Welfare Check (Backup)}
%==============================================================
\begin{frame}{Appendix: Is the Ranking Robust to a Genuine 2nd-Order Solution?}
%==============================================================

{\footnotesize \textbf{Why this was a caveat:} Rubbo's LQ welfare formula is 2nd-order in utility, but evaluated on \emph{1st-order} (certainty-equivalent) paths --- valid only if $\mathbb{E}[\tilde y_t]=\mathbb{E}[\pi_{it}]=0$ exactly. Two open-economy features can break that: the \textbf{debt-elastic UIP wedge} (nonlinear in $B^*_t$) and the \textbf{near-unit-root NFA} process --- both mean the true \emph{risk-adjusted} steady state can sit away from the deterministic one.}

\vspace{4pt}
{\footnotesize \textbf{Method:} the model is already coded nonlinear (levels) in Dynare. Solve to \textbf{order 2 with pruning}, \textbf{simulate} 260k periods (analytic order-$\ge2$ moments are unavailable: the Taylor rule leaves price levels/S with a unit root). Compute $\mathbb{E}[X^2]$ directly (not $\mathrm{Var}$) so any risk-adjusted mean shift is captured. Control for simulation noise with an identical-seed order=1 run.}

\vspace{6pt}
\begin{center}
{\scriptsize
\begin{tabular}{lcccc}
\toprule
\textbf{Welfare loss ($\times10^4$)} & \textbf{Float} & \textbf{Managed} & \textbf{Peg} \\
\midrule
1st-order (analytic, headline) & 25.47 & 10.17 & 102.05 \\
1st-order (simulated, same seed) & 25.58 & 10.20 & 102.39 \\
\textbf{2nd-order (pruned, simulated)} & \textbf{26.09} & \textbf{10.26} & \textbf{103.13} \\
\midrule
Net 2nd-order effect & $+2.0\%$ & $+0.6\%$ & $+0.7\%$ \\
\bottomrule
\end{tabular}}
\end{center}

\vspace{4pt}
{\footnotesize
\begin{itemize}\setlength{\itemsep}{3pt}
\item \textbf{Ranking survives}: Managed $<$ Float $<$ Peg at 2nd order, correction $\le2\%$ everywhere
\item \textbf{Risk-adjusted means are real but small}: e.g.\ Peg's output gap sits $\approx-45$bp on average (vs.\ $-2.5$bp Float, $-7$bp Managed) --- an order of magnitude larger, and NFA carries a precautionary buffer $\mathbb{E}[B^*_t]=+0.029$ under Peg vs.\ target $0$, largest where the UIP channel is least offset by $S_t$
\item \textbf{But squared loss barely notices}: $\mathbb{E}[X]^2/\mathbb{E}[X^2]<0.5\%$ in every regime --- almost all of the 2nd-order correction is a modest \textbf{variance amplification} from the pruned nonlinear policy function, not the risk-adjusted-mean channel
\end{itemize}}

\end{frame}

%==============================================================
\section*{Appendix: Divine Coincidence (DC) Index Details}
%==============================================================
\begin{frame}{Appendix: DC Index --- Definition and Insulation}
%==============================================================

{\scriptsize \textbf{Closed economy (Rubbo):} 1 channel (TFP), $\mathcal V$ rank-deficient $\Rightarrow$ unique DC index:}
\vspace{-3pt}
{\scriptsize\[ \bm\phi^\top\mathcal{V}=\bm 0^\top \Rightarrow \pi^{DC}_t = \textstyle\sum_i w^{DC}_i\pi_{it},\ w^{DC}_i\propto\lambda_{D,i}\tfrac{1-\hat\delta_i}{\hat\delta_i}\]}

{\scriptsize \textbf{Open economy: 2 channels} (TFP $+$ FX):}
\vspace{-3pt}
{\scriptsize\[ \underbrace{\bm\phi^\top\mathcal{V}=\bm 0^\top}_{\text{TFP}} \text{ and } \underbrace{\bm\phi^\top\Gamma = 0}_{\text{FX}} \Rightarrow \textbf{generically infeasible}\]}

\vspace{-2pt}
{\scriptsize \textbf{Knife-edge test:} $\Gamma\propto\mathcal{B}$ would save the DC index. Instead: $\cos(\Gamma,\mathcal{B})\approx0.96$ ($\approx17^\circ$ apart) --- close, but $\Gamma_i/\mathcal{B}_i$ shrinks $\approx$2$\times$ per sector, not constant. Breakdown is real, not a numerical artifact.
\textbf{Fix:} import nodes flexible ($\hat\delta_M{=}1\Rightarrow w_M{=}0$) $\Rightarrow$ DC well-defined again by construction --- what the Taylor rules target throughout ($\pi^{DC}_t$); FX insulation itself is a quantitative question, shown below.}

\vspace{-2pt}
\begin{center}
{\tiny \textbf{$\pi^{DC}_t$, import-price shock}}\\[0pt]
\begin{tikzpicture}
\begin{axis}[slide fig, width=8.5cm, height=2.5cm,
  xlabel={\tiny Quarters}, ylabel={\tiny $\pi^{DC}_t$ (\%)},
  xmin=0.5, xmax=10.5, ymin=-0.03, ymax=0.21, xtick={2,4,6,8,10},
  legend style={font=\tiny, at={(0.97,0.97)}, anchor=north east, row sep=-3pt}]
\addplot[thick, gray!40, dotted] coordinates {(1,0.1928)(2,0.1005)(3,0.0475)(4,0.0166)(5,-0.0014)(6,-0.0116)(7,-0.0172)(8,-0.0198)(9,-0.0208)(10,-0.0207)};
\addlegendentry{Peg}
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.0836)(2,0.0623)(3,0.0451)(4,0.0314)(5,0.0205)(6,0.0120)(7,0.0053)(8,0.0002)(9,-0.0037)(10,-0.0065)};
\addlegendentry{Managed}
\addplot[thick, customgreen!85!black] coordinates {(1,0.0264)(2,0.0222)(3,0.0185)(4,0.0153)(5,0.0126)(6,0.0103)(7,0.0084)(8,0.0068)(9,0.0055)(10,0.0044)};
\addlegendentry{Float}
\end{axis}
\end{tikzpicture}
\end{center}
{\scriptsize Float: smallest on impact (0.026\%) \& most persistent; Peg/Managed overshoot and reverse sign --- on-impact insulation $\ne$ full story.}

\end{frame}

%==============================================================
\section*{Appendix: Additional Shock IRFs (Backup, not in 25-slide count)}
%==============================================================
\begin{frame}{IRFs: Resource TFP Shock}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.1287, ymax=0.0168, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0129)(2,-0.0232)(3,-0.0291)(4,-0.0330)(5,-0.0356)(6,-0.0371)(7,-0.0378)(8,-0.0379)(9,-0.0376)(10,-0.0369)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0222)(2,-0.0419)(3,-0.0564)(4,-0.0685)(5,-0.0787)(6,-0.0874)(7,-0.0949)(8,-0.1014)(9,-0.1070)(10,-0.1119)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: TFP $A_{1t}$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.2955, ymax=1.0919, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.9000)(3,0.8100)(4,0.7290)(5,0.6561)(6,0.5905)(7,0.5314)(8,0.4783)(9,0.4305)(10,0.3874)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0053, ymax=0.0647, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0252)(2,0.0488)(3,0.0553)(4,0.0566)(5,0.0555)(6,0.0529)(7,0.0495)(8,0.0457)(9,0.0418)(10,0.0380)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.0065)(2,0.0171)(3,0.0210)(4,0.0231)(5,0.0242)(6,0.0246)(7,0.0245)(8,0.0241)(9,0.0234)(10,0.0226)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0027)(2,0.0086)(3,0.0097)(4,0.0099)(5,0.0097)(6,0.0093)(7,0.0088)(8,0.0082)(9,0.0076)(10,0.0069)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0488, ymax=0.0091, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-0.0269)(2,-0.0025)(3,0.0010)(4,0.0019)(5,0.0023)(6,0.0024)(7,0.0024)(8,0.0023)(9,0.0021)(10,0.0020)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0341)(2,-0.0090)(3,-0.0041)(4,-0.0022)(5,-0.0009)(6,0.0000)(7,0.0007)(8,0.0012)(9,0.0015)(10,0.0018)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0421)(2,-0.0170)(3,-0.0120)(4,-0.0098)(5,-0.0082)(6,-0.0070)(7,-0.0060)(8,-0.0051)(9,-0.0044)(10,-0.0039)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0249, ymax=0.0373, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.0054)(2,0.0035)(3,0.0047)(4,0.0048)(5,0.0046)(6,0.0044)(7,0.0041)(8,0.0038)(9,0.0035)(10,0.0032)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.0118)(2,0.0219)(3,0.0271)(4,0.0295)(5,0.0301)(6,0.0296)(7,0.0284)(8,0.0268)(9,0.0249)(10,0.0231)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0189)(2,0.0234)(3,0.0235)(4,0.0224)(5,0.0208)(6,0.0189)(7,0.0170)(8,0.0151)(9,0.0134)(10,0.0117)};
\addplot[thick, sectorres, dashed] coordinates {(1,-0.0058)(2,0.0030)(3,0.0042)(4,0.0043)(5,0.0043)(6,0.0041)(7,0.0039)(8,0.0037)(9,0.0034)(10,0.0031)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0079)(2,0.0156)(3,0.0207)(4,0.0236)(5,0.0250)(6,0.0253)(7,0.0248)(8,0.0239)(9,0.0227)(10,0.0213)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.0043)(2,-0.0015)(3,-0.0039)(4,-0.0049)(5,-0.0051)(6,-0.0048)(7,-0.0042)(8,-0.0034)(9,-0.0026)(10,-0.0018)};
\addplot[thick, sectorres] coordinates {(1,-0.0058)(2,0.0029)(3,0.0040)(4,0.0042)(5,0.0041)(6,0.0040)(7,0.0038)(8,0.0035)(9,0.0033)(10,0.0030)};
\addplot[thick, sectorman] coordinates {(1,0.0074)(2,0.0141)(3,0.0189)(4,0.0216)(5,0.0228)(6,0.0231)(7,0.0226)(8,0.0218)(9,0.0206)(10,0.0193)};
\addplot[thick, sectorserv] coordinates {(1,0.0012)(2,-0.0084)(3,-0.0132)(4,-0.0159)(5,-0.0173)(6,-0.0178)(7,-0.0176)(8,-0.0171)(9,-0.0163)(10,-0.0154)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-1.1411, ymax=0.2472, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.9550)(2,-0.0029)(3,0.0865)(4,0.0870)(5,0.0792)(6,0.0712)(7,0.0641)(8,0.0576)(9,0.0518)(10,0.0466)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.0266)(2,-0.0166)(3,-0.0091)(4,-0.0040)(5,-0.0007)(6,0.0014)(7,0.0027)(8,0.0034)(9,0.0037)(10,0.0038)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0013)(2,0.0014)(3,0.0012)(4,0.0011)(5,0.0009)(6,0.0007)(7,0.0005)(8,0.0004)(9,0.0002)(10,0.0001)};
\addplot[thick, sectorres, dashed] coordinates {(1,-0.9710)(2,-0.0156)(3,0.0795)(4,0.0829)(5,0.0769)(6,0.0702)(7,0.0639)(8,0.0581)(9,0.0528)(10,0.0479)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.0353)(2,-0.0246)(3,-0.0156)(4,-0.0091)(5,-0.0044)(6,-0.0012)(7,0.0011)(8,0.0026)(9,0.0035)(10,0.0041)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.0041)(2,-0.0037)(3,-0.0033)(4,-0.0028)(5,-0.0023)(6,-0.0019)(7,-0.0014)(8,-0.0011)(9,-0.0007)(10,-0.0004)};
\addplot[thick, sectorres] coordinates {(1,-0.9810)(2,-0.0259)(3,0.0702)(4,0.0743)(5,0.0689)(6,0.0628)(7,0.0570)(8,0.0517)(9,0.0468)(10,0.0423)};
\addplot[thick, sectorman] coordinates {(1,-0.0441)(2,-0.0333)(3,-0.0241)(4,-0.0172)(5,-0.0122)(6,-0.0085)(7,-0.0059)(8,-0.0039)(9,-0.0026)(10,-0.0016)};
\addplot[thick, sectorserv] coordinates {(1,-0.0116)(2,-0.0113)(3,-0.0107)(4,-0.0100)(5,-0.0093)(6,-0.0086)(7,-0.0079)(8,-0.0073)(9,-0.0066)(10,-0.0061)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}
%==============================================================
\begin{frame}{IRFs: Manufacturing TFP Shock}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.4750, ymax=0.1371, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.0664)(2,0.0055)(3,-0.0395)(4,-0.0721)(5,-0.0954)(6,-0.1116)(7,-0.1223)(8,-0.1288)(9,-0.1321)(10,-0.1331)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0555)(2,-0.0416)(3,-0.1208)(4,-0.1859)(5,-0.2397)(6,-0.2846)(7,-0.3223)(8,-0.3541)(9,-0.3812)(10,-0.4044)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: TFP $A_{2t}$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.2955, ymax=1.0919, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.9000)(3,0.8100)(4,0.7290)(5,0.6561)(6,0.5905)(7,0.5314)(8,0.4783)(9,0.4305)(10,0.3874)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.3695, ymax=0.2265, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-0.3008)(2,-0.1098)(3,0.0111)(4,0.0851)(5,0.1277)(6,0.1495)(7,0.1577)(8,0.1573)(9,0.1516)(10,0.1428)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0832)(2,-0.0332)(3,0.0029)(4,0.0284)(5,0.0460)(6,0.0578)(7,0.0654)(8,0.0698)(9,0.0719)(10,0.0723)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0417)(2,-0.0211)(3,-0.0066)(4,0.0031)(5,0.0094)(6,0.0133)(7,0.0155)(8,0.0164)(9,0.0166)(10,0.0162)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0926, ymax=0.0224, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-0.0582)(2,-0.0315)(3,-0.0152)(4,-0.0047)(5,0.0018)(6,0.0057)(7,0.0078)(8,0.0088)(9,0.0091)(10,0.0089)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0547)(2,-0.0518)(3,-0.0355)(4,-0.0232)(5,-0.0142)(6,-0.0075)(7,-0.0026)(8,0.0010)(9,0.0035)(10,0.0053)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0774)(2,-0.0794)(3,-0.0634)(4,-0.0510)(5,-0.0413)(6,-0.0339)(7,-0.0280)(8,-0.0234)(9,-0.0197)(10,-0.0167)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.3921, ymax=0.3025, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.0246)(2,-0.0116)(3,-0.0035)(4,0.0017)(5,0.0049)(6,0.0067)(7,0.0077)(8,0.0081)(9,0.0081)(10,0.0078)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.3120)(2,-0.1771)(3,-0.0869)(4,-0.0276)(5,0.0106)(6,0.0343)(7,0.0482)(8,0.0555)(9,0.0585)(10,0.0586)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0358)(2,0.0788)(3,0.1015)(4,0.1110)(5,0.1123)(6,0.1085)(7,0.1019)(8,0.0937)(9,0.0851)(10,0.0764)};
\addplot[thick, sectorres, dashed] coordinates {(1,-0.0188)(2,-0.0101)(3,-0.0036)(4,0.0008)(5,0.0037)(6,0.0055)(7,0.0066)(8,0.0071)(9,0.0073)(10,0.0072)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.2573)(2,-0.1575)(3,-0.0872)(4,-0.0381)(5,-0.0045)(6,0.0180)(7,0.0326)(8,0.0415)(9,0.0464)(10,0.0486)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.1929)(2,0.1344)(3,0.0937)(4,0.0657)(5,0.0468)(6,0.0343)(7,0.0262)(8,0.0212)(9,0.0182)(10,0.0165)};
\addplot[thick, sectorres] coordinates {(1,-0.0176)(2,-0.0097)(3,-0.0037)(4,0.0005)(5,0.0033)(6,0.0051)(7,0.0061)(8,0.0066)(9,0.0068)(10,0.0067)};
\addplot[thick, sectorman] coordinates {(1,-0.2464)(2,-0.1534)(3,-0.0876)(4,-0.0416)(5,-0.0099)(6,0.0114)(7,0.0254)(8,0.0340)(9,0.0389)(10,0.0411)};
\addplot[thick, sectorserv] coordinates {(1,0.2223)(2,0.1421)(3,0.0847)(4,0.0442)(5,0.0161)(6,-0.0032)(7,-0.0160)(8,-0.0242)(9,-0.0291)(10,-0.0316)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.3633, ymax=0.0909, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.0800)(2,0.0088)(3,0.0144)(4,0.0123)(5,0.0100)(6,0.0081)(7,0.0067)(8,0.0055)(9,0.0045)(10,0.0038)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.2823)(2,-0.1683)(3,-0.0915)(4,-0.0404)(5,-0.0072)(6,0.0139)(7,0.0268)(8,0.0340)(9,0.0375)(10,0.0385)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0014)(2,0.0037)(3,0.0046)(4,0.0048)(5,0.0045)(6,0.0040)(7,0.0034)(8,0.0027)(9,0.0021)(10,0.0015)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0060)(2,-0.0569)(3,-0.0452)(4,-0.0304)(5,-0.0191)(6,-0.0107)(7,-0.0048)(8,-0.0005)(9,0.0025)(10,0.0046)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.2857)(2,-0.1847)(3,-0.1129)(4,-0.0621)(5,-0.0266)(6,-0.0021)(7,0.0143)(8,0.0250)(9,0.0316)(10,0.0353)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.0070)(2,-0.0090)(3,-0.0096)(4,-0.0093)(5,-0.0085)(6,-0.0075)(7,-0.0063)(8,-0.0051)(9,-0.0039)(10,-0.0028)};
\addplot[thick, sectorres] coordinates {(1,-0.0014)(2,-0.0947)(3,-0.0834)(4,-0.0661)(5,-0.0521)(6,-0.0413)(7,-0.0331)(8,-0.0267)(9,-0.0218)(10,-0.0180)};
\addplot[thick, sectorman] coordinates {(1,-0.3109)(2,-0.2128)(3,-0.1423)(4,-0.0917)(5,-0.0557)(6,-0.0303)(7,-0.0127)(8,-0.0006)(9,0.0074)(10,0.0126)};
\addplot[thick, sectorserv] coordinates {(1,-0.0316)(2,-0.0346)(3,-0.0356)(4,-0.0354)(5,-0.0343)(6,-0.0327)(7,-0.0308)(8,-0.0288)(9,-0.0266)(10,-0.0245)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}
%==============================================================
\begin{frame}{IRFs: Services TFP Shock}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-1.0316, ymax=0.5851, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.3688)(2,0.2429)(3,0.1354)(4,0.0448)(5,-0.0308)(6,-0.0931)(7,-0.1439)(8,-0.1845)(9,-0.2164)(10,-0.2407)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.3986)(2,0.2010)(3,0.0200)(4,-0.1446)(5,-0.2937)(6,-0.4284)(7,-0.5498)(8,-0.6590)(9,-0.7571)(10,-0.8450)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: TFP $A_{3t}$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.2955, ymax=1.0919, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.9000)(3,0.8100)(4,0.7290)(5,0.6561)(6,0.5905)(7,0.5314)(8,0.4783)(9,0.4305)(10,0.3874)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-1.3549, ymax=0.4555, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-1.1460)(2,-0.7221)(3,-0.4164)(4,-0.1955)(5,-0.0374)(6,0.0736)(7,0.1496)(8,0.1996)(9,0.2302)(10,0.2466)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.1503)(2,-0.1046)(3,-0.0637)(4,-0.0280)(5,0.0025)(6,0.0282)(7,0.0496)(8,0.0671)(9,0.0811)(10,0.0921)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0652)(2,0.0471)(3,0.0343)(4,0.0247)(5,0.0174)(6,0.0120)(7,0.0078)(8,0.0047)(9,0.0023)(10,0.0005)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.1558, ymax=0.0382, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,-0.1095)(2,-0.0628)(3,-0.0363)(4,-0.0183)(5,-0.0057)(6,0.0030)(7,0.0088)(8,0.0125)(9,0.0147)(10,0.0158)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0316)(2,-0.0801)(3,-0.0670)(4,-0.0547)(5,-0.0438)(6,-0.0341)(7,-0.0256)(8,-0.0183)(9,-0.0120)(10,-0.0067)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0647)(2,-0.1334)(3,-0.1249)(4,-0.1155)(5,-0.1062)(6,-0.0973)(7,-0.0887)(8,-0.0807)(9,-0.0733)(10,-0.0664)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-1.3580, ymax=0.4862, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.0190)(2,0.0246)(3,0.0249)(4,0.0239)(5,0.0224)(6,0.0206)(7,0.0187)(8,0.0169)(9,0.0151)(10,0.0135)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.0198)(2,0.0384)(3,0.0725)(4,0.0909)(5,0.0991)(6,0.1008)(7,0.0984)(8,0.0936)(9,0.0875)(10,0.0808)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,-1.1452)(2,-0.7851)(3,-0.5139)(4,-0.3103)(5,-0.1589)(6,-0.0477)(7,0.0325)(8,0.0891)(9,0.1276)(10,0.1523)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0438)(2,0.0357)(3,0.0301)(4,0.0257)(5,0.0222)(6,0.0192)(7,0.0168)(8,0.0148)(9,0.0130)(10,0.0116)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.2213)(2,0.1777)(3,0.1450)(4,0.1203)(5,0.1015)(6,0.0871)(7,0.0758)(8,0.0669)(9,0.0598)(10,0.0539)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.4154)(2,-0.3180)(3,-0.2388)(4,-0.1741)(5,-0.1212)(6,-0.0781)(7,-0.0430)(8,-0.0146)(9,0.0083)(10,0.0266)};
\addplot[thick, sectorres] coordinates {(1,0.0491)(2,0.0386)(3,0.0318)(4,0.0266)(5,0.0224)(6,0.0191)(7,0.0164)(8,0.0141)(9,0.0123)(10,0.0107)};
\addplot[thick, sectorman] coordinates {(1,0.2734)(2,0.2125)(3,0.1664)(4,0.1316)(5,0.1052)(6,0.0850)(7,0.0695)(8,0.0575)(9,0.0481)(10,0.0407)};
\addplot[thick, sectorserv] coordinates {(1,-0.2573)(2,-0.2039)(3,-0.1639)(4,-0.1335)(5,-0.1102)(6,-0.0922)(7,-0.0781)(8,-0.0670)(9,-0.0581)(10,-0.0509)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.4627, ymax=0.2213, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,-0.3838)(2,0.0717)(3,0.0952)(4,0.0786)(5,0.0616)(6,0.0476)(7,0.0364)(8,0.0275)(9,0.0204)(10,0.0148)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,-0.0621)(2,-0.0230)(3,0.0003)(4,0.0130)(5,0.0190)(6,0.0208)(7,0.0203)(8,0.0184)(9,0.0159)(10,0.0132)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,-0.1336)(2,-0.0910)(3,-0.0587)(4,-0.0346)(5,-0.0166)(6,-0.0035)(7,0.0058)(8,0.0124)(9,0.0167)(10,0.0194)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0890)(2,-0.0458)(3,-0.0531)(4,-0.0469)(5,-0.0396)(6,-0.0327)(7,-0.0264)(8,-0.0208)(9,-0.0158)(10,-0.0115)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0065)(2,-0.0113)(3,-0.0215)(4,-0.0263)(5,-0.0277)(6,-0.0267)(7,-0.0244)(8,-0.0214)(9,-0.0180)(10,-0.0145)};
\addplot[thick, sectorserv, dashed] coordinates {(1,-0.1184)(2,-0.0944)(3,-0.0743)(4,-0.0575)(5,-0.0433)(6,-0.0314)(7,-0.0216)(8,-0.0134)(9,-0.0066)(10,-0.0010)};
\addplot[thick, sectorres] coordinates {(1,0.1424)(2,-0.1141)(3,-0.1348)(4,-0.1294)(5,-0.1207)(6,-0.1115)(7,-0.1026)(8,-0.0939)(9,-0.0857)(10,-0.0780)};
\addplot[thick, sectorman] coordinates {(1,-0.0285)(2,-0.0591)(3,-0.0782)(4,-0.0888)(5,-0.0935)(6,-0.0941)(7,-0.0920)(8,-0.0882)(9,-0.0834)(10,-0.0779)};
\addplot[thick, sectorserv] coordinates {(1,-0.1650)(2,-0.1456)(3,-0.1290)(4,-0.1146)(5,-0.1021)(6,-0.0911)(7,-0.0815)(8,-0.0729)(9,-0.0654)(10,-0.0587)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}
%==============================================================
\begin{frame}{IRFs: Foreign-Demand Shock}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.2869, ymax=0.0374, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.2013)(2,-0.1733)(3,-0.1492)(4,-0.1284)(5,-0.1104)(6,-0.0949)(7,-0.0815)(8,-0.0698)(9,-0.0597)(10,-0.0510)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.2495)(2,-0.2292)(3,-0.2112)(4,-0.1953)(5,-0.1812)(6,-0.1687)(7,-0.1576)(8,-0.1478)(9,-0.1390)(10,-0.1311)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: foreign demand $D^*_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.0044, ymax=1.1299, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.8000)(3,0.6400)(4,0.5120)(5,0.4096)(6,0.3277)(7,0.2621)(8,0.2097)(9,0.1678)(10,0.1342)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0913, ymax=0.7398, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.6439)(2,0.4713)(3,0.3459)(4,0.2529)(5,0.1838)(6,0.1324)(7,0.0941)(8,0.0656)(9,0.0445)(10,0.0288)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.1564)(2,0.1296)(3,0.1075)(4,0.0892)(5,0.0740)(6,0.0613)(7,0.0508)(8,0.0421)(9,0.0348)(10,0.0287)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0601)(2,0.0473)(3,0.0369)(4,0.0286)(5,0.0220)(6,0.0167)(7,0.0125)(8,0.0092)(9,0.0066)(10,0.0046)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0507, ymax=0.0505, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0388)(2,0.0204)(3,0.0118)(4,0.0062)(5,0.0024)(6,-0.0002)(7,-0.0019)(8,-0.0030)(9,-0.0036)(10,-0.0040)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0194)(2,0.0067)(3,0.0060)(4,0.0052)(5,0.0045)(6,0.0039)(7,0.0033)(8,0.0028)(9,0.0023)(10,0.0018)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0390)(2,-0.0049)(3,-0.0035)(4,-0.0026)(5,-0.0019)(6,-0.0013)(7,-0.0009)(8,-0.0007)(9,-0.0005)(10,-0.0003)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0747, ymax=0.5425, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.0173)(2,0.0102)(3,0.0069)(4,0.0047)(5,0.0033)(6,0.0022)(7,0.0015)(8,0.0010)(9,0.0007)(10,0.0004)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.1554)(2,0.1083)(3,0.0757)(4,0.0530)(5,0.0372)(6,0.0262)(7,0.0185)(8,0.0131)(9,0.0094)(10,0.0067)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.4713)(2,0.3528)(3,0.2633)(4,0.1952)(5,0.1434)(6,0.1040)(7,0.0741)(8,0.0515)(9,0.0344)(10,0.0217)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0055)(2,0.0044)(3,0.0035)(4,0.0029)(5,0.0023)(6,0.0019)(7,0.0015)(8,0.0012)(9,0.0010)(10,0.0008)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0393)(2,0.0339)(3,0.0292)(4,0.0251)(5,0.0215)(6,0.0184)(7,0.0158)(8,0.0135)(9,0.0116)(10,0.0099)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.1117)(2,0.0913)(3,0.0747)(4,0.0612)(5,0.0501)(6,0.0410)(7,0.0336)(8,0.0274)(9,0.0222)(10,0.0180)};
\addplot[thick, sectorres] coordinates {(1,0.0032)(2,0.0030)(3,0.0026)(4,0.0022)(5,0.0018)(6,0.0015)(7,0.0012)(8,0.0010)(9,0.0008)(10,0.0006)};
\addplot[thick, sectorman] coordinates {(1,0.0170)(2,0.0168)(3,0.0160)(4,0.0148)(5,0.0135)(6,0.0122)(7,0.0108)(8,0.0096)(9,0.0085)(10,0.0074)};
\addplot[thick, sectorserv] coordinates {(1,0.0398)(2,0.0275)(3,0.0183)(4,0.0116)(5,0.0066)(6,0.0030)(7,0.0004)(8,-0.0014)(9,-0.0027)(10,-0.0035)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0863, ymax=0.3218, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.2747)(2,-0.0139)(3,-0.0362)(4,-0.0322)(5,-0.0268)(6,-0.0222)(7,-0.0185)(8,-0.0155)(9,-0.0131)(10,-0.0111)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.0580)(2,0.0325)(3,0.0158)(4,0.0053)(5,-0.0011)(6,-0.0048)(7,-0.0068)(8,-0.0077)(9,-0.0079)(10,-0.0076)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0313)(2,0.0216)(3,0.0143)(4,0.0089)(5,0.0049)(6,0.0020)(7,-0.0001)(8,-0.0016)(9,-0.0026)(10,-0.0033)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0202)(2,0.0047)(3,0.0027)(4,0.0020)(5,0.0016)(6,0.0012)(7,0.0008)(8,0.0004)(9,0.0001)(10,-0.0001)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.0003)(2,0.0013)(3,0.0022)(4,0.0025)(5,0.0026)(6,0.0025)(7,0.0023)(8,0.0020)(9,0.0017)(10,0.0014)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.0054)(2,0.0047)(3,0.0040)(4,0.0034)(5,0.0029)(6,0.0024)(7,0.0019)(8,0.0015)(9,0.0011)(10,0.0008)};
\addplot[thick, sectorres] coordinates {(1,-0.0392)(2,-0.0067)(3,-0.0023)(4,-0.0013)(5,-0.0009)(6,-0.0006)(7,-0.0004)(8,-0.0003)(9,-0.0002)(10,-0.0001)};
\addplot[thick, sectorman] coordinates {(1,-0.0215)(2,-0.0148)(3,-0.0101)(4,-0.0067)(5,-0.0043)(6,-0.0027)(7,-0.0015)(8,-0.0008)(9,-0.0002)(10,0.0001)};
\addplot[thick, sectorserv] coordinates {(1,-0.0073)(2,-0.0062)(3,-0.0054)(4,-0.0046)(5,-0.0039)(6,-0.0034)(7,-0.0029)(8,-0.0025)(9,-0.0022)(10,-0.0019)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}
%==============================================================
\begin{frame}{IRFs: Export-Price / ToT Shock}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.3186, ymax=0.0416, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.2210)(2,-0.1981)(3,-0.1771)(4,-0.1579)(5,-0.1405)(6,-0.1246)(7,-0.1103)(8,-0.0974)(9,-0.0858)(10,-0.0753)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.2771)(2,-0.2671)(3,-0.2569)(4,-0.2468)(5,-0.2369)(6,-0.2274)(7,-0.2183)(8,-0.2096)(9,-0.2014)(10,-0.1937)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: export price $P^{X}_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.1164, ymax=1.1153, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.8500)(3,0.7225)(4,0.6141)(5,0.5220)(6,0.4437)(7,0.3771)(8,0.3206)(9,0.2725)(10,0.2316)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0928, ymax=0.7846, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.6834)(2,0.5182)(3,0.3954)(4,0.3019)(5,0.2303)(6,0.1751)(7,0.1326)(8,0.0996)(9,0.0740)(10,0.0542)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.1563)(2,0.1366)(3,0.1193)(4,0.1040)(5,0.0905)(6,0.0786)(7,0.0682)(8,0.0590)(9,0.0509)(10,0.0438)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0508)(2,0.0428)(3,0.0357)(4,0.0297)(5,0.0245)(6,0.0201)(7,0.0164)(8,0.0133)(9,0.0106)(10,0.0084)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0595, ymax=0.0586, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0450)(2,0.0253)(3,0.0157)(4,0.0092)(5,0.0046)(6,0.0014)(7,-0.0009)(8,-0.0024)(9,-0.0035)(10,-0.0042)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.0212)(2,0.0067)(3,0.0061)(4,0.0056)(5,0.0050)(6,0.0044)(7,0.0039)(8,0.0034)(9,0.0029)(10,0.0024)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.0459)(2,-0.0096)(3,-0.0076)(4,-0.0062)(5,-0.0050)(6,-0.0040)(7,-0.0033)(8,-0.0027)(9,-0.0022)(10,-0.0018)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0794, ymax=0.5764, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.0180)(2,0.0111)(3,0.0078)(4,0.0057)(5,0.0042)(6,0.0031)(7,0.0023)(8,0.0018)(9,0.0013)(10,0.0010)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.1646)(2,0.1188)(3,0.0866)(4,0.0636)(5,0.0472)(6,0.0354)(7,0.0269)(8,0.0207)(9,0.0161)(10,0.0127)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.5007)(2,0.3883)(3,0.3010)(4,0.2326)(5,0.1789)(6,0.1366)(7,0.1033)(8,0.0772)(9,0.0566)(10,0.0405)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0053)(2,0.0045)(3,0.0039)(4,0.0034)(5,0.0029)(6,0.0025)(7,0.0022)(8,0.0018)(9,0.0016)(10,0.0013)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0395)(2,0.0362)(3,0.0329)(4,0.0297)(5,0.0267)(6,0.0238)(7,0.0212)(8,0.0189)(9,0.0168)(10,0.0149)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.1114)(2,0.0958)(3,0.0824)(4,0.0709)(5,0.0609)(6,0.0522)(7,0.0447)(8,0.0382)(9,0.0326)(10,0.0277)};
\addplot[thick, sectorres] coordinates {(1,0.0029)(2,0.0030)(3,0.0028)(4,0.0025)(5,0.0023)(6,0.0020)(7,0.0017)(8,0.0015)(9,0.0013)(10,0.0011)};
\addplot[thick, sectorman] coordinates {(1,0.0153)(2,0.0169)(3,0.0174)(4,0.0172)(5,0.0165)(6,0.0155)(7,0.0144)(8,0.0133)(9,0.0121)(10,0.0110)};
\addplot[thick, sectorserv] coordinates {(1,0.0325)(2,0.0228)(3,0.0155)(4,0.0099)(5,0.0057)(6,0.0026)(7,0.0002)(8,-0.0015)(9,-0.0028)(10,-0.0037)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0992, ymax=0.3529, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.3007)(2,-0.0059)(3,-0.0321)(4,-0.0302)(5,-0.0262)(6,-0.0226)(7,-0.0196)(8,-0.0171)(9,-0.0149)(10,-0.0131)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.0667)(2,0.0393)(3,0.0209)(4,0.0089)(5,0.0012)(6,-0.0035)(7,-0.0063)(8,-0.0077)(9,-0.0084)(10,-0.0085)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0371)(2,0.0266)(3,0.0185)(4,0.0123)(5,0.0076)(6,0.0040)(7,0.0013)(8,-0.0007)(9,-0.0022)(10,-0.0032)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0216)(2,0.0056)(3,0.0035)(4,0.0028)(5,0.0023)(6,0.0018)(7,0.0014)(8,0.0010)(9,0.0006)(10,0.0003)};
\addplot[thick, sectorman, dashed] coordinates {(1,-0.0007)(2,0.0011)(3,0.0022)(4,0.0027)(5,0.0029)(6,0.0029)(7,0.0027)(8,0.0025)(9,0.0022)(10,0.0019)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.0063)(2,0.0055)(3,0.0048)(4,0.0042)(5,0.0036)(6,0.0030)(7,0.0025)(8,0.0020)(9,0.0016)(10,0.0012)};
\addplot[thick, sectorres] coordinates {(1,-0.0471)(2,-0.0120)(3,-0.0067)(4,-0.0050)(5,-0.0039)(6,-0.0031)(7,-0.0025)(8,-0.0021)(9,-0.0017)(10,-0.0014)};
\addplot[thick, sectorman] coordinates {(1,-0.0277)(2,-0.0204)(3,-0.0150)(4,-0.0109)(5,-0.0080)(6,-0.0058)(7,-0.0041)(8,-0.0029)(9,-0.0021)(10,-0.0014)};
\addplot[thick, sectorserv] coordinates {(1,-0.0107)(2,-0.0096)(3,-0.0085)(4,-0.0076)(5,-0.0067)(6,-0.0059)(7,-0.0053)(8,-0.0046)(9,-0.0041)(10,-0.0036)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}
%==============================================================
\begin{frame}{IRFs: Import-Price Shock (Backup)}
%==============================================================
\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Exch.\ rate $S_t$ (\%, dep.$>$0)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.6791, ymax=0.0886, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.0000)(2,0.0000)(3,0.0000)(4,0.0000)(5,0.0000)(6,0.0000)(7,0.0000)(8,0.0000)(9,0.0000)(10,0.0000)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,-0.4808)(2,-0.4063)(3,-0.3431)(4,-0.2893)(5,-0.2432)(6,-0.2037)(7,-0.1698)(8,-0.1408)(9,-0.1159)(10,-0.0945)};
\addplot[thick, customgreen!85!black] coordinates {(1,-0.5906)(2,-0.5270)(3,-0.4712)(4,-0.4219)(5,-0.3783)(6,-0.3396)(7,-0.3051)(8,-0.2742)(9,-0.2466)(10,-0.2218)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Shock: import price $P^{F*}_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=0.1164, ymax=1.1153, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, black] coordinates {(1,1.0000)(2,0.8500)(3,0.7225)(4,0.6141)(5,0.5220)(6,0.4437)(7,0.3771)(8,0.3206)(9,0.2725)(10,0.2316)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Output gap $\tilde y_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.2133, ymax=1.7236, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,1.5001)(2,1.0837)(3,0.7844)(4,0.5646)(5,0.4024)(6,0.2826)(7,0.1941)(8,0.1288)(9,0.0807)(10,0.0455)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.3270)(2,0.2714)(3,0.2257)(4,0.1875)(5,0.1554)(6,0.1285)(7,0.1058)(8,0.0867)(9,0.0705)(10,0.0569)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0938)(2,0.0754)(3,0.0604)(4,0.0482)(5,0.0382)(6,0.0301)(7,0.0235)(8,0.0181)(9,0.0137)(10,0.0102)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny CPI infl.\ $\pi^C_t$ (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.0548, ymax=0.2727, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, gray!45, densely dotted] coordinates {(1,0.2349)(2,0.0471)(3,0.0231)(4,0.0079)(5,-0.0022)(6,-0.0088)(7,-0.0129)(8,-0.0153)(9,-0.0166)(10,-0.0170)};
\addplot[thick, gray!60!black, dashed] coordinates {(1,0.0985)(2,0.0182)(3,0.0128)(4,0.0088)(5,0.0057)(6,0.0033)(7,0.0014)(8,-0.0001)(9,-0.0013)(10,-0.0022)};
\addplot[thick, customgreen!85!black] coordinates {(1,0.0576)(2,-0.0031)(3,-0.0034)(4,-0.0034)(5,-0.0032)(6,-0.0030)(7,-0.0027)(8,-0.0024)(9,-0.0021)(10,-0.0019)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
\begin{columns}[t]
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral output gap, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.1702, ymax=1.2737, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.0385)(2,0.0203)(3,0.0122)(4,0.0072)(5,0.0040)(6,0.0019)(7,0.0006)(8,-0.0002)(9,-0.0007)(10,-0.0009)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.3546)(2,0.2311)(3,0.1482)(4,0.0926)(5,0.0556)(6,0.0314)(7,0.0157)(8,0.0058)(9,-0.0001)(10,-0.0036)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,1.1071)(2,0.8323)(3,0.6240)(4,0.4648)(5,0.3428)(6,0.2493)(7,0.1778)(8,0.1231)(9,0.0815)(10,0.0500)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.0100)(2,0.0063)(3,0.0043)(4,0.0030)(5,0.0020)(6,0.0013)(7,0.0008)(8,0.0004)(9,0.0002)(10,0.0000)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0748)(2,0.0540)(3,0.0390)(4,0.0283)(5,0.0207)(6,0.0153)(7,0.0115)(8,0.0088)(9,0.0069)(10,0.0055)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.2422)(2,0.2112)(3,0.1824)(4,0.1562)(5,0.1327)(6,0.1119)(7,0.0935)(8,0.0775)(9,0.0635)(10,0.0514)};
\addplot[thick, sectorres] coordinates {(1,0.0046)(2,0.0030)(3,0.0020)(4,0.0013)(5,0.0008)(6,0.0004)(7,0.0002)(8,0.0000)(9,-0.0001)(10,-0.0002)};
\addplot[thick, sectorman] coordinates {(1,0.0207)(2,0.0129)(3,0.0078)(4,0.0046)(5,0.0026)(6,0.0015)(7,0.0010)(8,0.0008)(9,0.0008)(10,0.0010)};
\addplot[thick, sectorserv] coordinates {(1,0.0685)(2,0.0594)(3,0.0506)(4,0.0423)(5,0.0348)(6,0.0282)(7,0.0223)(8,0.0173)(9,0.0130)(10,0.0093)};
\end{axis}
\end{tikzpicture}
\end{center}
\column{0.5\textwidth}
\begin{center}
\begin{tikzpicture}
\begin{axis}[slide fig, title={\tiny Sectoral inflation, all regimes (\%)}, title style={yshift=-3pt, font=\tiny}, width=5.6cm, height=2.2cm, xlabel={\tiny Q}, xmin=0.5, xmax=10.5, ymin=-0.2242, ymax=0.9148, xtick={2,6,10}, yticklabel=\empty, scaled y ticks=false]
\addplot[thick, sectorres, densely dotted] coordinates {(1,0.7833)(2,-0.0262)(3,-0.0928)(4,-0.0853)(5,-0.0728)(6,-0.0619)(7,-0.0528)(8,-0.0452)(9,-0.0390)(10,-0.0337)};
\addplot[thick, sectorman, densely dotted] coordinates {(1,0.2039)(2,0.1166)(3,0.0587)(4,0.0216)(5,-0.0016)(6,-0.0154)(7,-0.0231)(8,-0.0268)(9,-0.0280)(10,-0.0276)};
\addplot[thick, sectorserv, densely dotted] coordinates {(1,0.0899)(2,0.0631)(3,0.0428)(4,0.0275)(5,0.0160)(6,0.0075)(7,0.0012)(8,-0.0034)(9,-0.0067)(10,-0.0089)};
\addplot[thick, sectorres, dashed] coordinates {(1,0.1753)(2,0.0271)(3,0.0068)(4,0.0014)(5,-0.0015)(6,-0.0036)(7,-0.0050)(8,-0.0060)(9,-0.0067)(10,-0.0071)};
\addplot[thick, sectorman, dashed] coordinates {(1,0.0681)(2,0.0462)(3,0.0302)(4,0.0187)(5,0.0106)(6,0.0048)(7,0.0007)(8,-0.0021)(9,-0.0040)(10,-0.0053)};
\addplot[thick, sectorserv, dashed] coordinates {(1,0.0309)(2,0.0258)(3,0.0212)(4,0.0172)(5,0.0136)(6,0.0105)(7,0.0078)(8,0.0055)(9,0.0036)(10,0.0019)};
\addplot[thick, sectorres] coordinates {(1,0.0382)(2,0.0077)(3,0.0026)(4,0.0008)(5,-0.0002)(6,-0.0008)(7,-0.0012)(8,-0.0014)(9,-0.0015)(10,-0.0015)};
\addplot[thick, sectorman] coordinates {(1,0.0238)(2,0.0143)(3,0.0078)(4,0.0035)(5,0.0007)(6,-0.0010)(7,-0.0021)(8,-0.0026)(9,-0.0028)(10,-0.0028)};
\addplot[thick, sectorserv] coordinates {(1,0.0067)(2,0.0058)(3,0.0050)(4,0.0042)(5,0.0034)(6,0.0028)(7,0.0022)(8,0.0017)(9,0.0013)(10,0.0009)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{columns}

\vspace{-6pt}
{\tiny Peg \textcolor{gray!55}{\textbf{\textbullet\textbullet\textbullet}}\quad Managed \textcolor{gray!60!black}{\textbf{- -}}\quad Float \textcolor{customgreen!85!black}{\textbf{---}}.\quad Sectoral by regime: line style $=$ regime, color $=$ \textcolor{sectorres}{Res.}\ \textcolor{sectorman}{Man.}\ \textcolor{sectorserv}{Serv.}}

\end{frame}

%==============================================================
\section*{Appendix: Model Equations (Backup, not in 25-slide count)}
%==============================================================
\begin{frame}{Appendix: Model Equations \& Notation (1/2)}
\hypertarget{modeleqns}{}
%==============================================================

{\scriptsize\textbf{Households}}
\vspace{-2pt}
{\tiny
\[
\max\ \mathbb{E}_0\sum_t \beta^t\Big[\tfrac{C_t^{1-\gamma}}{1-\gamma} - \tfrac{L_t^{1+\varphi}}{1+\varphi}\Big]
\quad\text{s.t.}\quad
P^C_tC_t + S_tB^*_t = W_tL_t + \Pi_t + (1{+}i^*)\big[1{-}\psi(\hat b^*_{t-1}{-}\bar b^*)\big]S_tB^*_{t-1}
\]
\vspace{-8pt}
\[
C_t^{-\gamma} = \beta(1{+}i_t)\mathbb{E}_t\big[C_{t+1}^{-\gamma}/\Pi^C_{t+1}\big],\quad
W_t/P^C_t = C_t^{\gamma}L_t^{\varphi},\quad
(1{+}i_t) = (1{+}i^*)\big[1{-}\psi(\hat b^*_t{-}\bar b^*)\big]\,\mathbb{E}_t[S_{t+1}/S_t]\ \text{(UIP)}
\]}
{\tiny
\begin{tabular}{@{}llll@{}}
$C,L$: consumption, labor & $\beta,\gamma,\varphi$: discount, RRA, inv.\ Frisch & $W,i$: wage, dom.\ rate & $S,i^*$: ER, foreign rate \\
$\psi$: debt-elastic premium & $B^*$: foreign bond & $P^C$: CPI & $\Pi_t$: profits (rebated) \\
\end{tabular}}

\vspace{10pt}
{\scriptsize\textbf{Firms: Technology and Cost}}
\vspace{-2pt}
{\tiny
\[
Y_{it}=A_{it}\,L_{it}^{\alpha_i}\prod_j X_{ijt}^{\Omega^H_{ij}}\,M_{it}^{\Omega^F_i}
\quad\Rightarrow\quad
MC_{it}=\frac{W_t^{\alpha_i}\prod_j P_{jt}^{\Omega^H_{ij}}\,(S_tP^{F*}_t)^{\Omega^F_i}}{A_{it}}
\]
\vspace{-6pt}
\[
W_tL_{it}=\alpha_i MC_{it}Y_{it},\quad P_{jt}X_{ijt}=\Omega^H_{ij}MC_{it}Y_{it},\quad S_tP^{F*}_tM_{it}=\Omega^F_i MC_{it}Y_{it}
\]}
{\tiny
\begin{tabular}{@{}llll@{}}
$Y,A$: output, TFP & $L,X,M$: labor, dom./imp.\ inputs & $\Omega^H_{ij}$: dom.\ IO share & $\Omega^F_i$: import cost share \\
$\alpha_i$: labor share & $MC_{it}$: nominal marg.\ cost & \multicolumn{2}{l}{$P^{F*}$: foreign import price} \\
\end{tabular}}

\vspace{6pt}
{\footnotesize \textbf{Key:} $MC_{it}$ moves with both $A_{it}$ (TFP) and $S_t$ (via $M_{it}$) --- the two cost-push channels.}

\end{frame}

%==============================================================
\begin{frame}{Appendix: Model Equations \& Notation (2/2)}
%==============================================================

{\scriptsize \textbf{Firms: Calvo Pricing} --- reset price (probability $\delta_i$ resets, $1{-}\delta_i$ keeps last price):}
\vspace{-4pt}
{\tiny
\[
X1_{it} = \widetilde{MC}_{it}Y_{it} + (1{-}\delta_i)\beta\Big(\tfrac{C_{t+1}}{C_t}\Big)^{-\gamma}\Pi_{i,t+1}^{\varepsilon}X1_{i,t+1},\quad
X2_{it} = Y_{it} + (1{-}\delta_i)\beta\Big(\tfrac{C_{t+1}}{C_t}\Big)^{-\gamma}\Pi_{i,t+1}^{\varepsilon-1}X2_{i,t+1}
\]
\vspace{-8pt}
\[
P_{it}^\#/P_{it} = \tfrac{\varepsilon}{\varepsilon-1}\,X1_{it}/X2_{it},\quad
1 = (1{-}\delta_i)\Pi_{it}^{\varepsilon-1} + \delta_i \big(P_{it}^\#/P_{it}\big)^{1-\varepsilon},\quad
\hat\delta_i=\dfrac{\delta_i(1-\beta(1-\delta_i))}{1-\beta\delta_i(1-\delta_i)}
\]}
{\tiny
\begin{tabular}{@{}llll@{}}
$\delta_i$: Calvo reset prob.\ (primitive) & $\hat\delta_i$: derived slope (enters DC weights) & $P_{it}^\#$: reset price & $\varepsilon$: elasticity (markup $\varepsilon/(\varepsilon{-}1)$) \\
\end{tabular}}

\vspace{10pt}
{\scriptsize\textbf{Open Economy}}
\vspace{-2pt}
{\tiny
\[
\underbrace{P^{FH}_t = S_t P^{F*}_t}_{\text{LOP}},\quad
\underbrace{(1{+}i_t) = (1{+}i^*)\big[1{-}\psi(\hat b^*_t{-}\bar b^*)\big]\,\mathbb{E}_t[S_{t+1}/S_t]}_{\text{UIP}}
\]
\vspace{-8pt}
\[
\hat b^*_t = \tfrac{1+i_{t-1}}{\Pi^C_t}\hat b^*_{t-1} + \tfrac{P^H_t EX_t - P^{FH}_t IM_t}{P^C_t},\quad
EX_t = \kappa_{EX}D^*_t\Big(\tfrac{P^H_t}{P^{FH}_t}\Big)^{-\theta^*}
\]
\vspace{-8pt}
\[
Y_{it} = \beta^H_i\tfrac{P^H_t}{P_{it}}\big(C^H_t+EX_t\big) + \sum_j \Omega^H_{ji}\tfrac{MC_{jt}Y_{jt}}{P_{it}}
\]}
{\tiny
\begin{tabular}{@{}llll@{}}
$\hat b^*$: NFA (scaled) & $EX,IM$: exports, imports & $\theta^*$: export elasticity & $D^*$: foreign demand shifter \\
$\beta^H_i$: cons.\ share, sector $i$ & \multicolumn{3}{l}{Float: $i_t$ Taylor rule, $S_t$ free \quad Peg: $S_t=\bar S$, $i_t$ residual} \\
\end{tabular}}

\end{frame}

%==============================================================
\section*{Appendix: Extended Literature Review (Backup)}
%==============================================================
\begin{frame}{Appendix: The Peg vs.\ Float Debate --- What Do We Know?}
\hypertarget{littfull}{}
%==============================================================

{\footnotesize \textbf{Why it depends} (structure):}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Mundell (1961)} OCA --- fix favored by small, open, factor-mobile economies w/ correlated shocks vs.\ anchor; float favored by large, diversified ones
\item \textbf{Frankel (1999)} --- ``no single regime is right for all countries or at all times''; openness, size, trade \& capital-market structure all matter
\end{itemize}}

\vspace{2pt}
{\footnotesize \textbf{Shock-absorption view} (why float wins on real shocks --- robustly):}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Friedman (1953)} --- $S_t$ adjusts instead of costly wage/price deflation
\item \textbf{Broda (2004)} --- 75 dev.\ countries, 1973--96: negative ToT shocks $\to$ large real GDP declines under pegs
\item \textbf{Edwards \& Levy-Yeyati (2005)} --- 10\% ToT deterioration: $-0.80$pp growth under pegs vs.\ $-0.43$pp under floats
\item \textbf{Céspedes, Chang \& Velasco (2004)} --- even with dollarized-liability balance-sheet effects, float \emph{still} beats fixed
\end{itemize}}

\vspace{2pt}
{\footnotesize \textbf{But floating is not a free lunch} (limits on float's insulation):}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Calvo \& Reinhart (2002)} ``Fear of Floating'' --- EMEs peg de facto anyway: credibility-building, not literal welfare optimum
\item \textbf{Rey (2013)} ``Dilemma not trilemma'' --- under free capital mobility, float alone doesn't deliver monetary autonomy
\end{itemize}}

\vspace{3pt}
{\scriptsize \textbf{Consensus:} no corner solution is universally optimal --- the choice trades a nominal anchor against real-shock absorption, shock- and structure-dependent.}

\end{frame}

%==============================================================
\begin{frame}{Appendix: The Frontier --- Is Fear of Floating Actually Optimal?}
%==============================================================

{\scriptsize
\begin{itemize}\setlength{\itemsep}{5pt}
\item \textbf{Itskhoki \& Mukhin (2023)} --- general open-economy framework with endogenous UIP/PPP deviations: efficient policy $=$ MP closes the output gap $+$ FX intervention eliminates the UIP wedge. Optimal policy is generically a \textbf{managed float/crawling peg}, matching observed fear of floating. When the natural RER is stable, this collapses to an ``open-economy divine coincidence.''
\\ \textcolor{customgreen!55!black}{$\hookrightarrow$ \emph{vs.\ us:}} \textbf{same headline} (Managed, not a corner, is optimal) --- reached instead via a multi-sector Calvo network with a persistence-driven, not UIP-gap-closing, mechanism

\item \textbf{Bianchi \& Coulibaly (2025; 2022 WP)} --- fear of floating is \textbf{optimal}, not behavioral, when credit access is tied to the real exchange rate (self-fulfilling crisis risk). Prescription: float for real shocks, \textbf{fix} for financial shocks
\\ \textcolor{customgreen!55!black}{$\hookrightarrow$ \emph{vs.\ us:}} \textbf{opposite ranking} for our financial shock --- our risk-premium/UIP shock passes straight through under \textbf{Peg} (worst regime). Different financial friction (persistent NFA wedge vs.\ collateral-constraint crisis) flips the prescription

\item \textbf{Coulibaly (2023)} --- discretionary MP is procyclical to offset balance-sheet effects; commitment (inflation targeting) or capital controls raise welfare
\\ \textcolor{customgreen!55!black}{$\hookrightarrow$ \emph{vs.\ us:}} echoes our ``Managed $=$ extra instrument'' logic --- giving policy more tools beats a corner regime
\end{itemize}}

\vspace{4pt}
{\scriptsize \textbf{Bottom line:} the 2023--25 frontier has also moved past corner solutions --- but \emph{why} fear of floating is optimal is mechanism-specific. We add production-network amplification $+$ shock persistence as a distinct reason, not examined by either paper.}

\end{frame}

%==============================================================
\begin{frame}{Appendix: How Our Results Compare to the Literature}
%==============================================================

{\scriptsize
\begin{tabular}{@{}p{2.8cm}p{3.55cm}p{3.55cm}@{}}
\toprule
\textbf{Literature} & \textbf{Says} & \textbf{Us} \\
\midrule
Mundell (1961); Frankel (1999) & Optimal regime depends on structure, no universal answer & \textbf{Confirms}: ranking flips with shock type (UIP vs.\ TFP) and network density \\[3pt]
Friedman (1953); Broda (2004); Edwards--Levy-Yeyati (2005); Céspedes--Chang--Velasco (2004) & Float buffers real (ToT/foreign) shocks, lower output loss than peg --- robust even to balance-sheet effects & \textbf{Same direction} (Peg worst), \textbf{different shock}: risk-premium/UIP, not ToT \\[3pt]
Gal\'i \& Monacelli (2005) & Peg dominated, float $\approx$ optimal in 1-sector SOE & \textbf{Same ranking of Peg}, but pure \textbf{Float is not optimal}: Managed beats it \\[3pt]
Calvo--Reinhart (2002) & EMEs avoid pure float --- read as \textbf{fear}, a credibility-driven deviation from optimum & We find partial stabilization is \textbf{literally welfare-optimal} --- a persistence-driven, not fear-driven, reason \\[3pt]
Rey (2013) & Float alone doesn't insulate; need a second instrument (capital controls/macropru) & Echoes Fanelli--Straub: Managed uses $S_t$ itself as that \textbf{second instrument} \\[3pt]
Qiu, Wang, Xu \& Zanetti (2026, JME) & Open-economy DC index (CPI+expenditure-switching+profit channels), \emph{static}, WIOD-calibrated; DC breaks down, output-gap targeting near-optimal & \textbf{Independent corroboration} DC breaks down in open economy; \textbf{we add} the piece they flag as their own top open extension --- UIP, NFA persistence, and FX-\textbf{regime choice} \\
\bottomrule
\end{tabular}}

\vspace{6pt}
{\footnotesize \textbf{Novel angle:} the classic shock-absorber logic (Friedman/Broda) is about shock \textbf{type} (real vs.\ nominal). Our driver is a shock \textbf{property} --- persistence (near-unit-root NFA) --- not previously emphasized in this literature.}

\end{frame}

%==============================================================
\section*{Appendix: ECB Open-Economy Production-Network Model (Backup)}
%==============================================================
\begin{frame}{Appendix: The ECB's Open-Economy Production-Network Model}
\hypertarget{ecbnetwork}{}
%==============================================================

{\scriptsize \textbf{Gnocato, Montes-Gald\'on \& Stamato (2025)}, ``Tariffs across the supply chain,'' ECB WP No.\ 3081 --- also underlies Lane's (ECB Exec.\ Board) 13 May 2026 speech on energy shocks (4-region extension):}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Setup:} two-region, multi-sector, full IO structure, international production network; sticky wages/prices; Taylor rule on \emph{domestic} CPI (no regime comparison)
\item \textbf{Finding:} tariffs on \textbf{intermediate} inputs $\to$ persistent inflation \& larger GDP loss (low substitutability, network propagation); tariffs on \textbf{final} goods $\to$ one-off inflation spike, smaller GDP loss --- \emph{where} in the network a shock hits sets its persistence
\item \textcolor{customgreen!55!black}{$\hookrightarrow$ \emph{vs.\ us:}} their tariff-location result and our TFP-vs-UIP result are the same insight in two settings: shock \textbf{location/type} sets its persistence, echoing our ``driver is persistence, not volatility''
\end{itemize}}

\vspace{2pt}
{\scriptsize \textbf{What it cites for open economy $+$ production networks:}}
{\scriptsize
\begin{itemize}\setlength{\itemsep}{1pt}
\item Network amplification, closed economy: \textbf{Pasten, Schoenle \& Weber (2020)}; \textbf{Ghassibe (2021)}; \textbf{Rubbo (2023)}\footnote{\tiny Published version: Rubbo, E. (2023), Econometrica 91(4), 1417--1455 --- journal year is 2023, not 2024.}; \textbf{Afrouzi \& Bhattarai (2023)}
\item Open-economy multi-sector NK $+$ networks: \textbf{Devereux, Gente \& Yu (2023)}; \textbf{Hinterlang et al.\ (2023)} EMuSe (Bundesbank); closest: \textbf{Kalemli-\"Ozcan, Soylu \& Yildirim (2025)} ``Global networks, MP and trade''
\item Trade fragmentation \& inflation: \textbf{Moro \& Nispi Landi (2024)}; \textbf{Ambrosino, Chan \& Tenreyro (2026, \emph{JIE})}
\item Optimal MP under tariffs: \textbf{Bergin \& Corsetti (2023)}; \textbf{Bianchi \& Coulibaly (2025)} ``Optimal MP response to tariffs'' (\emph{different} paper from their fear-of-floating one)
\end{itemize}}

\vspace{3pt}
{\scriptsize \textbf{Bottom line:} none compares FX \textbf{regimes} or asks whether a DC index survives --- they fix the monetary rule. That gap is still exactly our question.}

\end{frame}

\end{document}
