...
%==============================================================
\vspace{-6pt}
{\scriptsize Reparametrize $\bm\omega_F$ only, on the REAL Chile calibration ($\alpha_i=1-\Omega^H_i-\omega_{F,i}$ absorbs it, holding $\Omega^H$ fixed at its Chile values):
\[
...
\vspace{-4pt}
{\scriptsize \textbf{Managed $<$ Float $<$ Peg robust throughout both sweeps} (real Chile calibration, $\zeta=\theta_M=1$ recovers the headline 25.47/10.17/102.05 exactly); gap still moves in \textbf{opposite directions}, though not symmetrically:
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item $\zeta$ (openness): Peg $\downarrow$ ($153.7\to65.8$), Float $\uparrow$ ($23.1\to36.0$), Managed U-shaped (min.\ near $\zeta{=}1$) --- \textbf{converge}
\item $\theta_M$ (concentration): Peg $\uparrow$ much faster ($94.0\to107.3$, $+14\%$) than Float ($25.2\to25.8$, $+2\%$) or Managed ($9.8\to10.4$) --- \textbf{gap still diverges}, but via Peg rising, not Float falling (Float is nearly flat, unlike the earlier stylized-network sweep, which had Float declining with concentration)
\item \textbf{Mechanism:} $\alpha_i+\Omega^H_i+\omega_{F,i}=1$ links the labor- and import-cost channels within every sector. $\zeta$ moves both economy-wide (Float's $\Gamma$-channel vs.\ Peg's labor-channel); $\theta_M$ moves them in/out of Services ($\approx$all the Domar weight) --- Peg's risk-premium/output-gap channel is far more sensitive to this reallocation than Float's price-dispersion channel
\end{itemize}}
...
%==============================================================
\vspace{-6pt}
{\scriptsize Same construction as "Openness \& Import Concentration," applied to the FULL sector-export model's $\kappa_{EX,i}$ (real Chile calibration, not stylized):
\[
...
\vspace{2pt}
\begin{center}
\includegraphics[width=0.98\textwidth]{figs/export_openness_concentration.pdf}
\end{center}

\vspace{-4pt}
{\scriptsize \textbf{Managed $<$ Float $<$ Peg robust throughout both sweeps.} Unlike import $\zeta$ (which moves Peg/Float in \emph{opposite} directions --- $\omega_{F,i}$ is a cost share, trading off against $\alpha_i$), export $\zeta$ moves \textbf{all three regimes together}:
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{0pt}
\item $\zeta$ (openness): \textbf{all regimes improve} as trade opens further --- Peg most ($98.7\to84.8$, $-14\%$), Float/Managed least ($\approx-4\%$) --- since $\kappa_{EX,i}$ only enters export \emph{demand}, not the cost side, so it never trades off against the labor share the way $\omega_{F,i}$ does
\item $\theta_X$ (concentration): \textbf{opposite directions, as with import concentration} --- Peg $\downarrow$ ($94.1\to89.6$), Float/Managed $\uparrow$ ($24.8\to25.8$, $10.6\to11.2$) as exports concentrate toward the true exporter (Resource, the flexible-price sector)
\item \textbf{Mechanism:} routing exports toward Resource's low price-dispersion weight $\kappa_1$ helps Peg (its loss is dominated by the risk-premium/output-gap channel, and a more flexible-priced export sector absorbs shocks better); the same reallocation raises Float/Managed's dispersion cost slightly since more of \emph{their} adjustment now runs through Resource's inflation instead of being spread via consumption shares
\end{itemize}}
...
{\scriptsize
\begin{itemize}\setlength{\itemsep}{-2pt}\setlength{\topsep}{0pt}
\item \textbf{Managed is least sensitive, not neutral:} $\rho{=}0\to1$ adds $+1.17$ to Managed vs.\ $+5.52$ Float, $+25.72$ Peg (20.0/9.0/76.3 $\to$ 25.5/10.2/102.1)
\item \textbf{Peg's rise is a pure output-gap story:} its per-sector price dispersion \emph{falls} slightly at every sector ($-0.01/-3.62/-1.82$) as $\rho{:}0\to1$ --- the $+25.72$ comes entirely from the output-gap term amplifying with network density, not price dispersion
\item \textbf{Float/Managed's rise concentrates in Services} ($+4.90$/$+0.64$), matching Services' large Domar weight
\item \textbf{No convergence at high density} (unlike the earlier stylized-network estimate): Peg/Float ratio \emph{grows} from $4.0\times$ ($\rho{=}1$) to $5.2\times$ ($\rho{=}2.2$, 40.1/16.5/206.8) --- denser networks widen, not narrow, the gap between corners
\item \textbf{Verdict:} Peg--Managed gap roughly \textbf{doubles} from $\rho{:}1\to2.2$ (91.9$\to$190.3) --- the network channel matters even more on real data than the stylized estimate suggested
\end{itemize}}
...
\item \textbf{Optimal $\phi_s\approx0.20$} (real Chile calibration): interior optimum, but nearly flat vs.\ the calibrated $0.30$ ($\approx$0.3\% apart) --- over-stabilizing $S_t$ beyond $\approx0.3$ is what's costly
\item \textbf{Ranking robust} to $\zeta,\theta_M,\rho$ (all three now Chile-calibrated) \& to Korea/Czechia calibrations; the \emph{gap} is not
\item \textbf{Network channel isolated} (Chile calibration, $\rho$ dial on all six cross-sector $\Omega^H$ links): $\rho{=}0\to1$ adds $+1.17$ to Managed vs.\ $+5.52$ Float, $+25.72$ Peg --- \textbf{Peg's rise is entirely an output-gap story} (its price dispersion falls slightly at every sector); Peg does \emph{not} converge toward Float at high density, unlike an earlier stylized-network estimate
...
{\footnotesize \textbf{Why this was a caveat:} the "75\% risk-premium" result rests on a debt-elastic UIP closing device (Schmitt-Grohé--Uribe 2003) at $\psi=0.020$. Known pitfall: $\psi$ near zero can create a near-unit-root NFA blowup that's a linearization artifact --- the order-2 check (previous slide) ruled out spurious \emph{nonlinearity}, not whether \emph{this} $\psi$ value does unreasonable work.}

\vspace{3pt}
{\footnotesize \textbf{Method:} scale $\psi$ over $0.25\times$--$4\times$ baseline (0.005--0.080), re-solve all three regimes (nonlinear, Dynare), recompute the risk-premium shock's \% share of welfare loss via the exact variance decomposition.}
...
\midrule
\textbf{Network premium} & \textbf{+213\%} & \textbf{+92\%} & \textbf{+169\%} \\
\bottomrule
\end{tabular}}
\end{center}
...
{\footnotesize
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{Network roughly doubles-to-triples every regime's loss} --- not a decorative feature, most acutely Float ($+213\%$)
\item \textbf{Ranking preserved either way}: Managed $<$ Float $<$ Peg with \emph{and} without the network
\item \textbf{Network \emph{reallocates} Peg's loss}: without it, output-gap collapses ($79.53\to10.53$) but price dispersion \emph{rises} ($22.51\to27.41$)
\end{itemize}}

\end{frame}

%==============================================================
\section*{Appendix: Sector-Specific Export Demand (Full Model)}
%==============================================================
\begin{frame}{Appendix: Does Export \emph{Concentration} Matter, Not Just Export Openness?}
%==============================================================

{\footnotesize \textbf{Why this is needed:} the headline model allocates aggregate exports $EX_t$ via household shares $\beta_i^H$ (Services-heavy: $0.744$) --- \emph{opposite} Chile's real export pattern (Resource-heavy: $0.602/0.180/0.036$). FULL version: three sector-specific equations $EX_{i,t}=\kappa_{EX,i}D^*_t P_t^X(P_{i,t}/P_t^{FH})^{-\theta^*}$, $\kappa_{EX,i}$ calibrated (Newton's method) so exports hit the true exporting sector.}
...
\bottomrule
\end{tabular}\\[3pt]
{\tiny Export/ToT shock welfare ($\times10^4$), old$\to$new: Float $0.11\to0.17$, Managed $0.18\to0.14$, Peg $2.16\to1.17$}}
\end{center}
...
\item \textbf{Concentrating exports in the flexible-price sector \emph{lowers} Peg's ToT cost}: Resource ($\delta_1=0.90$, low $\kappa_1$) vs.\ Services (stickiest, high $\kappa_3$) cuts Peg's ToT welfare cost $46\%$
...
\vspace{-4pt}
\begin{center}
{\tiny \textbf{Welfare loss vs.\ $\phi_s$ ($\times10^4$), full 7-shock model, real Chile calibration}}\\[1pt]
\begin{tikzpicture}
