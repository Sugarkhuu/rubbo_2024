...
// reverse flows) taken from Chile's actual 2023-vintage national input-
// output table -- see data_calibration/build_chile_calibration.py and
// data_calibration/chile_calibration_results.json for the full source,
...
//
// CALIBRATED FROM DATA (Banco Central de Chile, Cuadros 12x12, CdeR2018,
// reference year 2023): OH11-OH33 (domestic IO matrix), OF1-OF3 (import
// cost shares), BH1-BH3 (household consumption shares), ALPHA1-ALPHA3
// (value-added shares, standing in for the model's labor share since
// there is no separate capital factor).
...
// against data_calibration/build_chile_calibration.py's numpy computation
// (LAMBDA_D = [0.0718, 0.3820, 1.1084], MIMP = [0.1547, 0.2847, 0.1189]).
// -------------------------------------------------------------------------
M11 = 1 - OH11;
M12 = -OH12;
M13 = -OH13;
...
// -------------------------------------------------------------------------
// ORDER-2 PRUNED SIMULATION (Kim-Kim-Schaumburg pruning). Analytic
// (theoretical) moments at order>=2 are not requested because P1-3/PC/S/
// GDP carry a unit root (no price-level anchor in the Taylor rule) and
// Dynare's pruned_state_space_system assumes stationarity. Simulating
// instead sidesteps that: empirical moments of the STATIONARY variables
// (piDC, PI1-3, y_gap, BSTAR, I, S) are well defined from a long
// pruned simulation regardless of the unit root elsewhere in the state.
stoch_simul(order=2, pruning, periods=260000, drop=10000, nograph) piDC PIC y_gap y_gap1 y_gap2 y_gap3 PI1 PI2 PI3 I BSTAR S;
