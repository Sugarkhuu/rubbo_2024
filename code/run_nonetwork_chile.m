% =========================================================================
% TASK #1 (todo_three_exercises.txt): Network vs. no-network counterfactual
% on the REAL Chile calibration (not the stylized triangular-network
% proxy used by the netdens sweep).
%
% Zeroes out ALL nine entries of the Chile Omega^H matrix (OH11..OH33,
% including the diagonal -- no self-loops either, matching the definition
% already used in run_sweep_point.m's 'netdens' sweep), and lets ALPHA_i
% absorb the freed-up cost share: ALPHA_i = 1 - 0 - OF_i. OF_i (import
% shares) and BH_i (consumption shares) are left at their real Chile data
% values. Re-solves the steady state via the existing dense solver
% (soe_ss_solve_dense.m handles an all-zero OH matrix fine -- it's a
% general solver, M = I - Omega^H just becomes the identity) and
% re-simulates all three regimes.
%
% Run from the REPO ROOT:
%   >> addpath('C:\dynare\6.3\matlab'); addpath('code'); run_nonetwork_chile
% =========================================================================
...
out_dir = 'results_chile_nonetwork';
...
for r = 1:numel(regimes)
    ...
    % NOTE: MATLAB's "." matches newlines by default (unlike most regex
    % flavors), so a trailing ".*" here would swallow the rest of the
    % FILE, not just the rest of the line -- use [^\n]* to stay in-line.
    ...
    fname = sprintf('oen_chile_nonet_%s', regime);
    fid = fopen([fname '.mod'], 'w'); fwrite(fid, txt); fclose(fid);

    fprintf('\n=== no-network Chile, regime: %s ===\n', regime);
