% =========================================================================
% Run the DENSE-Omega^H, Korea-data-calibrated nonlinear model
% (open_economy_network_korea*.mod) under all three FX regimes and export
% IRFs, theoretical variances, and derived network objects to
% results_korea/*.csv -- mirrors code/run_all_regimes.m exactly, just
% pointed at the Korea-calibrated .mod files and a separate output folder
% so the original (stylized-calibration) results/ is untouched.
%
% Run from the REPO ROOT (not code/):
%   >> addpath('C:\dynare\6.3\matlab'); addpath('code'); run_korea_regimes
% =========================================================================

regimes = {'float', 'peg', 'managed'};
modfiles = struct('float', 'open_economy_network_korea', ...
                   'peg', 'open_economy_network_korea_peg', ...
                   'managed', 'open_economy_network_korea_managed');
