% =========================================================================
% Run the DENSE-Omega^H, Chile-data-calibrated nonlinear model
% (open_economy_network_chile*.mod) under all three FX regimes and export
% IRFs, theoretical variances, and derived network objects to
% results_chile/*.csv -- mirrors code/run_all_regimes.m exactly, just
% pointed at the Chile-calibrated .mod files and a separate output folder
% so the original (stylized-calibration) results/ is untouched.
%
% Run from the REPO ROOT (not code/):
%   >> addpath('C:\dynare\7.0\matlab'); addpath('code'); run_chile_regimes
% =========================================================================

regimes = {'float', 'peg', 'managed'};
modfiles = struct('float', 'open_economy_network_chile', ...
                   'peg', 'open_economy_network_chile_peg', ...
                   'managed', 'open_economy_network_chile_managed');
