% Chile-calibrated replacement for code/sweep_phi_s.m (which used the
% stylized open_economy_network_managed.mod). Re-solves the managed-float
% regime for a grid of PHI_S values on the REAL Chile calibration
% (open_economy_network_chile_managed.mod). PHI_S does not enter the
...
% Run from repo root: addpath('C:\dynare\6.3\matlab'); addpath('code'); sweep_phi_s_chile

phi_s_grid = [0.00, 0.10, 0.20, 0.30, 0.50, 0.75, 1.00, 1.50, 2.00];
% Must match the stoch_simul var_list order in open_economy_network_chile_managed.mod
...
master_txt = fileread('open_economy_network_chile_managed.mod');
rows = {};

for k = 1:numel(phi_s_grid)
    phi_s = phi_s_grid(k);
    txt = regexprep(master_txt, 'PHI_S\s*=\s*0\.30;', sprintf('PHI_S   = %.6f;', phi_s));
    fname = sprintf('open_economy_network_chile_phis_%d', k);
...
fid = fopen(fullfile('results', 'phi_s_sweep_chile.csv'), 'w');
fprintf(fid, 'phi_s,%s\n', strjoin(moment_vars, ','));
for k = 1:numel(rows)
    r = rows{k};
...
fclose(fid);
fprintf('Wrote results/phi_s_sweep_chile.csv\n');
